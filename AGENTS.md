...
- 開発日記へ進捗を追記するときは、小型の`.video-trigger`を添え、共通`#video-modal`で対応するテストシナリオ動画を再生できるようにする。動画は固定カメラまたは再現可能な操作から生成し、標準テスト・性能測定ではキャプチャを無効にする。同一動画を参照できる場合はファイルを複製せず、連番PNGは`artifacts/`だけに置く。動画だけを成功判定には使わない。
- 画面で確認可能な変更を安全な区切りまで完了した場合、その変更を最も理解しやすい短いPhase別デモを生成し、実際に再生確認してから開発日誌の「最新デモ」を更新する。固定シナリオへ限定せず、内部変更だけで映像上の差がない場合は無理に生成せず報告する。
