"""Run exactly one Phase 6IB actual-Kit stage-open smoke."""
...
CASE = SCRIPTS / "run_phase6ib_stage_open_case.ps1"
PROBE = SCRIPTS / "probe_phase6ib_stage_open_smoke.py"
...
def _contract() -> tuple[dict, str]:
    ...
    if digest != expected:
        raise RuntimeError("Phase 6IB contract digest mismatch")
    return json.loads(CONTRACT.read_text(encoding="utf-8")), digest


def run_smoke(root: Path, preflight_path: Path) -> dict:
    if root.exists():
        raise RuntimeError("Phase 6IB smoke refuses root reuse")
    ...
    if preflight.get("status") != "qualified" or preflight.get("contract_sha256") != digest:
        raise RuntimeError("Phase 6IB preflight did not qualify this contract")
    ...
    attempt_id = "phase6ib-stage-open-attempt01"
    paths = {
    ...
    target = [
        ...
        "-StageRoot", str(attempt / "generated-stages"),
    ...
    write_json(attempt / "launch_contract.json", {
        "schema": "campfire.phase6ib.launch.v1", "phase": "phase6ib", "mode": "stage_open_smoke",
        "attempt_id": attempt_id, "target": target, "cwd": str(ROOT),
        "timeline_play_flow_interface_readback_capture_forbidden": True,
    })
    command = build_guard_command(
        PYTHON, GUARD, paths, target, attempt_id=attempt_id, safety=policy["safety"], include_gpu=True
    )
    ...
    counters_zero = all(audit.get(key) == 0 for key in ("timeline_play_calls", "flow_interface_calls", "readback_calls", "capture_calls"))
    accepted = bool(
        ...
        and counters_zero and runner.get("status") == "qualified" and roles_ok
        and before == after and cleanup.get("all_observed_absent") is True
    )
    markers = runner.get("marker_names") or []
    result = {
        "schema": "campfire.phase6ib.stage-open-summary.v1", "phase": "phase6ib",
        ...
        "marker_names": markers, "parser_fixture": parser_fixture, "stage": stage,
        "forbidden_operation_counters_zero": counters_zero,
        ...
        "phase6ia_reclassified": False, "phase6ia_artifact_or_runtime_reused": False,
