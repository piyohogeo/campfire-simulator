...
for ($run = 1; $run -le $RunCount; $run++) {
    ...
    & $phase3Runner -OutputDir $runOutput -ArrayBackend python -CellStateStorage dict -ConstantHeatCapacityPath original -ProfileWoodInternals -PythonSurfaceBoundaryPath fast -PythonStateClampPath fast -CellPhaseUpdates deferred -RuntimeMetrics compact -RuntimeTopology dynamic
