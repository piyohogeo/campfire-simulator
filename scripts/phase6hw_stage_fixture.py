"""No-Kit generated-stage fixture for the Phase 6HW one-variable comparison."""
...
from phase6hw_stage_builder import TOKEN_OFF, TOKEN_ON, write_stage
from phase6hw_stage_contract import validate_stage, validate_stage_bytes
...
def run_fixture(output_root: Path, contract_path: Path) -> dict:
    if output_root.exists():
        raise RuntimeError("Phase 6HW stage fixture refuses root reuse")
    ...
    _case(cases, "diagnostic_geometry_is_bounded", off["evidence"]["topology"] == [26, 36, 120] and off["evidence"]["gap_in_velocity_voxels"] >= 1.0 and off["evidence"]["end_clearance_in_velocity_voxels"] >= 16.0)
    _case(cases, "flow_only_display", off["gates"]["no_opaque_render_mesh"] and on["gates"]["no_opaque_render_mesh"])
    wrong = validate_stage_bytes(off_path.read_bytes(), contract, "collision_on")
    _case(cases, "wrong_condition_rejected", not wrong["passed"], validation=wrong)
    ...
    _case(cases, "non_collision_mutation_rejected", not mutation["passed"], validation=mutation)
    _case(cases, "bounded_fixture_files", all(path.stat().st_size < 1024 * 1024 for path in generated.iterdir()))
    report = {
        "schema": "campfire.phase6hw.stage-authoring-fixture.v1",
        "phase": "phase6hw",
