"""Phase 6GB deterministic Kit wrapper for the frozen Phase 6FO physics probe."""
...
try:
    shared, import_audit = load_exact_module(
        SHARED_PATH,
        SHARED_PATH,
        module_name="campfire_phase6gb_shared_supply_probe",
        required_entrypoints=("_run", "_append_resource_marker"),
    )
    _write_audit({
        "schema": "campfire.phase6gb.kit-import-audit.v1",
        "status": "pass",
...
def _synchronized_append(path, marker, *args, **kwargs):
    ...
    while time.monotonic() - started < timeout:
        if acknowledgement.is_file():
            return
        if failure.is_file():
            raise RuntimeError("Phase 6GB pre-close measurement committer failed")
        time.sleep(0.05)
    raise TimeoutError("Phase 6GB pre-close measurement commit acknowledgement timed out")
