...
$analyzer = Join-Path $PSScriptRoot "analyze_resident_lightweight_commit.py"

if (-not (Test-Path -LiteralPath $kitPython)) { throw "Application is not built." }
if (-not $OutputDir) {
    $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6bg-repro"
}
...
for ($run = 1; $run -le $RunCount; $run++) {
    $baselineDir = Join-Path $OutputDir ("transaction-{0:D2}" -f $run)
    $candidateDir = Join-Path $OutputDir ("lightweight-{0:D2}" -f $run)
    $baselineArgs = @{
