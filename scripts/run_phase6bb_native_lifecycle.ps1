...
$benchmark = Join-Path $PSScriptRoot "benchmark_native_backend_lifecycle.py"
$analyzer = Join-Path $PSScriptRoot "analyze_native_backend_lifecycle.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6bb" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$rawReport = Join-Path $OutputDir "native_backend_lifecycle_raw.json"
...
& $kitPython $benchmark --dll $nativeDll --logs 20 --initial-steps $InitialSteps --runs $RunCount --output $rawReport
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
