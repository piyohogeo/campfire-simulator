"""Phase 6HE one-variable velocity sub-boundary probe."""
...
if CONDITION not in ROW_BY_NAME:
    raise ImportError(f"unsupported Phase 6HE condition: {CONDITION!r}")
ROW = ROW_BY_NAME[CONDITION]
LEVEL = int(ROW["mode"][1:])

hb, hb_audit = load_exact_module(
    HB_PATH, HB_PATH, module_name="campfire_phase6he_phase6hb_base", required_entrypoints=()
)
...
def append_resource_marker(path, name, *args, **kwargs):
    translated = name.replace("phase6hb_", "phase6he_", 1) if name.startswith("phase6hb_") else name
    if kwargs.get("phase") == "phase6hb":
        kwargs["phase"] = "phase6he"
...
def velocity_boundary(flow, volume_provider, arguments, frame, output, collectors, operation_state=None):
    ...
    def marker(name: str, **values) -> None:
        canonical = name if name.startswith("phase6he_") else f"phase6he_{name}"
    ...
    volume_row = None
    grid = None
    metadata_root = output.parent / f"phase6he-schema-f{int(frame):04d}"
    ...
    try:
        ...
        if LEVEL >= 2:
            ...
            marker("velocity_pipeline_before", diagnostic_stop_after=ROW["stop_after"])
            velocity_result = hb.shared._save_and_sample(
                ...
                diagnostic_stop_after=ROW["stop_after"],
        ...
        for key in (
            "velocity_save", "velocity_sampling", "velocity_collector",
            ...
            if hb.report["calls"][key] != 0:
                raise RuntimeError(f"prohibited broad/temperature counter is nonzero: {key}")
        complete_operation(hb.report)
        write_operation_report(hb.REPORT_PATH, hb.report)
        return {
            "mode": f"phase6he_{ROW['mode']}",
...
hb._atomic_json(hb.AUDIT_PATH, {
    "schema": "campfire.phase6he.kit-import-audit.v1",
    "status": "pass",
    ...
    "velocity_stop_after": ROW["stop_after"],
