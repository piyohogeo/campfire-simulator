...
$analyzer = Join-Path $PSScriptRoot "analyze_resident_point_continuity_fix.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6cn" }
...
& $kit @(
    ...
    "--/exts/campfire.app/residentPointContinuityFixQualificationEnabled=true",
    "--/exts/campfire.app/residentNativeLibraryPath=$nativeDll",
    "--/rtx/flow/enabled=true"
)
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$result = Get-Content -LiteralPath $raw -Raw | ConvertFrom-Json
if ($result.status -ne "ok") { throw "Phase 6CN qualification failed: $raw" }
$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_continuity_fix_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_continuity_fix_report.svg"
$poster = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_continuity_fix_frame.png"
$video = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_continuity_fix.mp4"
& $kitPython $analyzer --raw $raw --report $report --svg $svg --poster $poster --frames $videoFrames
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$ffmpeg = Get-Command ffmpeg.exe -ErrorAction SilentlyContinue
if (-not $ffmpeg) { throw "ffmpeg.exe is required to encode the Phase 6CN video." }
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host ("Phase 6CN qualified: gates={0}, playing samples={1}, max alignment error={2} m" -f @($result.gates.PSObject.Properties).Count, $result.known_issue.timeline_playing_sample_count, $result.known_issue.maximum_observed_alignment_error_m)
