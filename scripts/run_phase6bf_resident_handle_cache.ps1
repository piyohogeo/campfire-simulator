...
$analyzer = Join-Path $PSScriptRoot "analyze_resident_handle_cache.py"

if (-not (Test-Path -LiteralPath $kitPython)) { throw "Application is not built." }
if (-not $OutputDir) {
    $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6bf-repro"
}
...
for ($run = 1; $run -le $RunCount; $run++) {
    $baselineDir = Join-Path $OutputDir ("baseline-{0:D2}" -f $run)
    $candidateDir = Join-Path $OutputDir ("candidate-{0:D2}" -f $run)
    if (($run % 2) -eq 1) {
        & $phase3Runner -OutputDir $baselineDir -ResidentSnapshotAdapter
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & $phase3Runner -OutputDir $candidateDir -ResidentSnapshotAdapter -ResidentSnapshotHandleCache
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }
    else {
        & $phase3Runner -OutputDir $candidateDir -ResidentSnapshotAdapter -ResidentSnapshotHandleCache
