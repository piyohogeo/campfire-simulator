...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6FL refuses artifact root reuse: $OutputRoot" }
$contractPath = if ([string]::IsNullOrWhiteSpace($ContractPath)) {
    Join-Path $PSScriptRoot "phase6fl_three_iteration_contract.json"
...
if ($actualHash -ne $expectedHash) { throw "Phase 6FL contract hash mismatch" }
...
foreach ($item in $contract.runtime_implementation) {
    ...
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $path).Hash -ne [string]$item.sha256) {
        throw "Phase 6FL runtime implementation hash changed: $($item.path)"
    }
}
...
if ((Get-FileHash -Algorithm SHA256 -LiteralPath $basePath).Hash -ne $contract.base_operation_contract.sha256) {
    throw "Phase 6FL base operation contract changed"
}
...
$analyzer = Join-Path $PSScriptRoot "analyze_phase6fl_three_iteration.py"
...
$reportPath = Join-Path $OutputRoot "three_iteration_report.json"
...
function Write-State([string]$Status, [string]$ActiveAttempt, [string]$ActiveSlot, [string]$Classification, [string]$Reason) {
    $payload = [ordered]@{
        schema = "campfire.phase6fl.incremental-state.v1"; phase = "phase6fl"; status = $Status
...
function Update-Report {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 6FL analyzer failed" }
}
function Assert-Production([string]$Boundary) {
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $productionApp).Hash -ne $productionBefore) {
        ...
        throw "Phase 6FL production app changed"
    }
}
...
while ($slotIndex -lt $slots.Count -and $attempted -lt [int]$contract.population.maximum_launches) {
    ...
    $mode = [string]$spec.readback_mode
    $caseDir = Join-Path $attemptRoot $label
    $metadata = [ordered]@{
        schema = "campfire.phase6fl.attempt-metadata.v1"; attempt_id = $attemptId
        ...
        operation_frames = @($contract.operation_frames); timestamp_utc = [DateTime]::UtcNow.ToString("o")
    }
    ...
    try { Update-Report } catch {
        ...
        Write-Error "Phase 6FL analyzer failed after $attemptId"
        exit 2
    }
    ...
    Write-Error "Phase 6FL captured nonreplaceable $classification at $attemptId; later slots were not started"
    exit 2
}
...
Write-Host "Phase 6FL completed nine representative three-iteration processes; iteration count above three remains excluded"
