...
S = ROOT / "scripts"
CONTRACT = S / "phase6ij_stage_open_composition_contract.json"
SIDECAR = S / "phase6ij_stage_open_composition_contract.sha256"
PYTHON = Path(r"C:\Python38\python.exe")
GUARD = S / "phase6hr_resource_guard.py"
CASE = S / "run_phase6ij_stage_open_composition_case.ps1"
PROBE = S / "probe_phase6ij_stage_open_composition.py"


def _attempt(root: Path, condition: str, policy: dict) -> dict:
    attempt_id = "phase6ij-stage-open-" + condition.lower()
    ...
    write_json(attempt / "launch_contract.json", {"schema": "campfire.phase6ij.launch.v1", "phase": "phase6ij", "attempt_id": attempt_id, "condition": condition, "target": target, "cwd": str(ROOT)})
    ...
    marker_identity = ladder.validate_marker_identity_consistency(marker_rows, operation)
    ...
    qualified = (
        operation_result["accepted"] and marker_result["accepted"] and marker_identity["accepted"]
        and runner.get("process_exit_code") == 0 and canonical.get("accepted")
        and cleanup.get("all_observed_absent") is True and roles_ok
        and not runner.get("fatal_lines") and not runner.get("dump_inventory")
        and not runner.get("automatic_upload_attempt_lines")
    )
    return {
        "condition": condition, "attempt_id": attempt_id, "qualified": qualified,
        "guard_exit_code": guard_exit, "operation_validation": operation_result,
        "marker_validation": marker_result, "marker_identity_validation": marker_identity,
        "operation_report": operation, "opened_stage_identity": operation.get("observed_identity") if operation else None,
        "kit_exit_code": runner.get("process_exit_code"),
        "lifecycle_classification": canonical.get("classification"), "lifecycle_reason": canonical.get("reason"),
...
def _is_native_failure(row: dict) -> bool:
    if row["dump_inventory"] or row["fatal_lines"] or row["kit_exit_code"] == 3221225477:
        return True
    error = str(row.get("error") or "").lower()
    return "0xc0000005" in error or "access violation" in error or "native exception" in error


def _is_composition_specific(row: dict, prior: list[dict]) -> bool:
    if row["condition"] not in ("B", "C") or not prior or not all(item["qualified"] for item in prior):
        return False
    if _is_native_failure(row):
        return False
    reasons = row.get("operation_validation", {}).get("reasons", [])
    deterministic = ("root_identifier_mismatch", "sublayer_identifiers_mismatch", "runtime_empty_state_mismatch", "open_path_mismatch")
    return any(any(token in reason for token in deterministic) for reason in reasons)


def run(root: Path, preflight_path: Path) -> dict:
    if root.exists():
        raise RuntimeError("Phase 6IJ runtime refuses root reuse")
    ...
    if digest != SIDECAR.read_text().split()[0].upper() or preflight.get("status") != "qualified" or preflight.get("contract_sha256") != digest:
        raise RuntimeError("Phase 6IJ preflight/contract invalid")
    root.mkdir(parents=True)
    for source, target in ((CONTRACT, "frozen_contract.json"), (SIDECAR, "frozen_contract.sha256"), (S / "phase6ij_authoring_dependencies.json", "frozen_dependency_manifest.json"), (S / "phase6ij_authoring_dependencies.sha256", "frozen_dependency_manifest.sha256")):
        shutil.copy2(source, root / target)
    before = invariant_hashes()
    attempts = []
    for condition in policy["operation_contract"]["condition_order"]:
        result = _attempt(root, condition, policy)
        attempts.append(result)
        write_json(root / "aggregate.partial.json", {"phase": "phase6ij", "attempts": attempts})
    ...
    elif failed and _is_native_failure(failed):
        status = "safe_stop_stage_open_native_failure_unlocalized"
    elif failed and _is_composition_specific(failed, attempts[:-1]):
        status = "safe_stop_stage_open_composition_specific_failure"
    else:
        status = "safe_stop_stage_open_contract_failure"
    result = {
        "schema": "campfire.phase6ij.summary.v1", "phase": "phase6ij", "status": status,
        "contract_sha256": digest, "condition_order": ["A", "B", "C"], "requested_D_mapping": "C",
        "attempts": attempts, "kit_launch_count": sum(row["kit_launch_count"] for row in attempts),
        ...
        "phase6ii_reclassified": False, "phase6ii_artifact_attempt_reused": False, "phase6ii_rerun": False,
