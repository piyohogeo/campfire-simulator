...
$benchmark = Join-Path $PSScriptRoot "benchmark_native_arrhenius_complete_step.py"
$analyzer = Join-Path $PSScriptRoot "analyze_native_arrhenius_complete_step.py"
$vswhere = "${env:ProgramFiles(x86)}\Microsoft Visual Studio\Installer\vswhere.exe"

if (-not (Test-Path -LiteralPath $kitPython)) { throw "Application is not built." }
if ($WarmupSteps -ge $Steps) { throw "WarmupSteps must be smaller than Steps." }
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6ax" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$rawReport = Join-Path $OutputDir "native_arrhenius_complete_step_raw.json"
...
& $kitPython $benchmark --dll $nativeDll --logs 20 --steps $Steps --warmup $WarmupSteps --runs $RunCount --output $rawReport
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
