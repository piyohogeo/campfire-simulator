...
$producer=[IO.Path]::GetFullPath($ProducerPath);$schema=[IO.Path]::GetFullPath($SchemaPath);$systemPython=[IO.Path]::GetFullPath($SystemPythonPath)
...
$arguments=@($app,"--no-window","--/app/file/ignoreUnsavedOnExit=true","--/app/fastShutdown=0","--/app/quitAfter=300000",
 ...
 "--/phase6hs/attemptId=$AttemptId","--/phase6hs/expectedCampfirePath=$ExpectedCampfirePath","--/phase6hs/expectedAnimPath=$ExpectedAnimPath",
...
$evidence=[ordered]@{schema="campfire.phase6hu.kit-case-runner.v1";phase="phase6hu";condition="collision_off";status=if($qualified){"qualified"}else{"failed"};failure=$failure
 ...
 process_exit_code=if($null-eq$monitor){$null}else{$monitor.exit_code};shutdown_monitor=$monitor;run_status=if($null-eq$run){"missing"}else{$run.status}
 canonical_producer_exit_code=$producerExit;canonical_output_path=$canonicalOutput;raw_output_path=$rawOutput
