"""Phase 6GA deterministic Kit wrapper for the frozen Phase 6FO physics probe."""
...
try:
    shared, import_audit = load_exact_module(
        SHARED_PATH,
        SHARED_PATH,
        module_name="campfire_phase6ga_shared_supply_probe",
        required_entrypoints=("_run", "_append_resource_marker"),
    )
    _write_audit({
        "schema": "campfire.phase6ga.kit-import-audit.v1",
        "status": "pass",
...
def _synchronized_append(path, marker, *args, **kwargs):
    ...
    while time.monotonic() - started < timeout:
        if acknowledgement.is_file():
            return
        if failure.is_file():
            raise RuntimeError("Phase 6GA pre-close measurement committer failed")
        time.sleep(0.05)
    raise TimeoutError("Phase 6GA pre-close measurement commit acknowledgement timed out")
