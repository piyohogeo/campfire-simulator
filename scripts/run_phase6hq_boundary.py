"""Run one Phase 6HQ smoke or proxy boundary with canonical lifecycle policy."""
...
CONTRACT = ROOT / "scripts/phase6hq_cleanup_assisted_proxy_contract.json"
SIDECAR = ROOT / "scripts/phase6hq_cleanup_assisted_proxy_contract.sha256"
INVARIANTS = {
...
def _contract() -> tuple[dict, str]:
    ...
    if digest != expected:
        raise RuntimeError("Phase 6HQ contract digest mismatch")
...
def main() -> int:
    ...
    if root.exists():
        raise RuntimeError("Phase 6HQ boundary refuses root reuse")
    ...
    if precondition.get("status") not in ("qualified", "pass"):
        raise RuntimeError("Phase 6HQ precondition did not pass")
    if args.mode == "proxy":
        if precondition.get("mode") != "smoke" or precondition.get("canonical_lifecycle_classification") not in policy["accepted_classifications"]:
            raise RuntimeError("proxy requires accepted fresh Phase 6HQ smoke")
    ...
    write_json(attempt / "launch_contract.json", {
        "schema": "campfire.phase6hq.launch.v1", "phase": "phase6hq", "mode": args.mode,
    ...
    attempt_id = "phase6hq-" + args.mode + "-attempt01"
    command = build_guard_command(
        Path(r"C:\Python38\python.exe"), ROOT / "scripts/phase6hq_resource_guard.py",
    ...
    passed = bool(
        canonical["accepted"] and guard_exit == 0 and run and run.get("status") == "qualified"
        and case and case.get("status") == "qualified" and roles_ok and invariants_pass
    )
    evaluation = canonical.get("evaluation") or {}
    cleanup = {} if guard is None else guard.get("observed_process_cleanup") or {}
    summary = {
        "schema": "campfire.phase6hq.boundary-summary.v1", "phase": "phase6hq", "mode": args.mode,
        ...
        "canonical_consumer_reason": canonical["reason"],
        "canonical_lifecycle_classification": canonical["classification"],
        "natural_clean_exit": evaluation.get("natural_exit") is True,
        "cleanup_assisted_exit": evaluation.get("cleanup_intervention") is True,
        "cleanup_helper": evaluation.get("helper"),
        "cleanup_killed_pids": cleanup.get("killed_pids"),
        ...
        "phase6hp_reclassified": False, "phase6hp_artifact_reused": False,
