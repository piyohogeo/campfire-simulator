...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6EX refuses artifact root reuse: $OutputRoot" }
New-Item -ItemType Directory -Path $OutputRoot | Out-Null
$logs = Join-Path $OutputRoot "runner-logs"
New-Item -ItemType Directory -Path $logs | Out-Null

$contractPath = Join-Path $PSScriptRoot "phase6ex_r0_stability_contract.json"
$hashPath = Join-Path $PSScriptRoot "phase6ex_r0_stability_contract.sha256"
...
if ($actualHash -ne $expectedHash) { throw "Phase 6EX contract hash mismatch" }
...
$analyzer = Join-Path $PSScriptRoot "analyze_phase6ex_r0_stability.py"
$fixture = Join-Path $PSScriptRoot "run_phase6ex_sampler_fixture.py"
...
$reportPath = Join-Path $OutputRoot "r0_stability_report.json"
$statePath = Join-Path $OutputRoot "incremental_state.json"
$limits = $contract.safety
$window = $contract.stability_observation
...
function Write-State([string]$Status, [int]$Completed, [string]$Active, [string]$Reason) {
    ...
    $state = [ordered]@{
        schema="campfire.phase6ex.incremental-state.v1"; phase="phase6ex"; status=$Status
        completed_kit_processes=$Completed; maximum_kit_processes=[int]$contract.execution.maximum_kit_processes
...
function Update-Report {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 6EX analyzer failed" }
}

function Stop-Safely([int]$Completed, [string]$Active, [string]$Reason) {
    Write-State "safe_stop" $Completed $Active $Reason
    Update-Report
    Write-Error "Phase 6EX safe stop at ${Active}: $Reason"
    exit 2
}

function Invoke-Case([string]$Label, [string]$Relative, [string]$Prefix, [int]$RunIndex, [string]$Mode, [string]$ReadbackFrames) {
    ...
    $arguments = @(
        ...
        "-Collision", "true", "-Policy", "allow_self_center", "-ReportPhase", "phase6ex",
        "-GeometryVariant", "phase6er_corrected", "-FuelScale", "1", "-TemperatureScale", "1", "-SmokeScale", "1",
        "-SampleFrames", "30,60,90,120,150,180,200,240,280,320", "-ReadbackChannels", "none", "-ReadbackMode", $Mode,
    ...
    if (-not $case.plateau) { return "incremental_plateau_gate_failed" }
    if ($case.stability_resource_sample_count -lt [int]$contract.plateau_contract.minimum_resource_samples_in_stability_interval) { return "stability_sample_minimum_failed" }
    if ($Mode -eq "acquire_discard" -and $null -eq $case.acquire_private_bytes.before) { return "acquire_markers_missing" }
    return $null
}

$completed = 0
Write-State "running" $completed "sampler_fixture" ""
& python $fixture --output (Join-Path $OutputRoot "sampler-fixture") `
    --duration-seconds $window.extra_running_flow_wall_seconds `
    --sample-seconds $window.outer_resource_sample_seconds `
    --target-samples $window.target_resource_samples `
    --minimum-samples $window.minimum_resource_samples
if ($LASTEXITCODE -ne 0) { Stop-Safely $completed "sampler_fixture" "sampler_fixture_failed" }
...
for ($run = 1; $run -le 3; $run++) {
    ...
    $reason = Invoke-Case $label $relative $prefix $run "none" ""
...
$reason = Invoke-Case "R1_acquire_discard" "R1_acquire_discard" "R1_acquire_discard" 1 "acquire_discard" "60"
...
Write-Host "Phase 6EX completed: sampler fixture, R0 3/3 extended plateau, and one bounded R1 acquisition qualified"
