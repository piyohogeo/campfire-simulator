...
$analyzer = Join-Path $PSScriptRoot "analyze_resident_transform_observer.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6cl" }
...
& $kit @(
    ...
    "--/exts/campfire.app/residentPointTransformQualificationEnabled=true",
    "--/exts/campfire.app/residentNativeLibraryPath=$nativeDll",
    "--/rtx/flow/enabled=true"
)
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$result = Get-Content -LiteralPath $raw -Raw | ConvertFrom-Json
if ($result.status -ne "ok") { throw "Phase 6CL failed: $raw" }
$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_transform_observer_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_transform_observer_report.svg"
$poster = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_transform_observer_frame.png"
$video = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_transform_observer.mp4"
& $kitPython $analyzer --raw $raw --report $report --svg $svg --poster $poster --frames $videoFrames
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$ffmpeg = Get-Command ffmpeg.exe -ErrorAction SilentlyContinue
if (-not $ffmpeg) { throw "ffmpeg.exe is required to encode the Phase 6CL video." }
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host ("Phase 6CL qualified: gates={0}, requests={1}, coalesced={2}" -f @($result.gates.PSObject.Properties).Count, $result.lifecycle.command_queue.request_count, $result.lifecycle.command_queue.coalesced_submission_count)
