...
for ($pair = 1; $pair -le $PairCount; $pair++) {
    ...
    foreach ($backend in $backends) {
        ...
        & $phase3Runner -OutputDir $runOutput -ArrayBackend $backend -PythonSurfaceBoundaryPath original -PythonStateClampPath original -CellPhaseUpdates eager -RuntimeMetrics full -RuntimeTopology dynamic
