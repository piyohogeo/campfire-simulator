...
if (-not $EagerProfileSummary -and -not $DeferredProfileSummary) {
    ...
    & $phase3Runner -OutputDir $eagerProfileOutput -ArrayBackend python -ProfileWoodInternals -PythonSurfaceBoundaryPath fast -PythonStateClampPath fast -CellPhaseUpdates eager
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    & $phase3Runner -OutputDir $deferredProfileOutput -ArrayBackend python -ProfileWoodInternals -PythonSurfaceBoundaryPath fast -PythonStateClampPath fast -CellPhaseUpdates deferred
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
for ($pair = 1; $pair -le $PairCount; $pair++) {
    ...
    foreach ($pathName in $paths) {
        ...
        & $phase3Runner -OutputDir $runOutput -ArrayBackend python -PythonSurfaceBoundaryPath fast -PythonStateClampPath fast -CellPhaseUpdates $pathName
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
