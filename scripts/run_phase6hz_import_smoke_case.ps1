...
$arguments=@($app,"--no-window","--/app/file/ignoreUnsavedOnExit=true","--/app/fastShutdown=0","--/app/quitAfter=120000",
 ...
 "--/exts/campfire.app/autoCreateScene=false","--/phase6hz/markers=$markers","--/phase6hz/importAudit=$audit","--/phase6hz/attemptId=$AttemptId",
...
$policy=Get-Content -Raw -Encoding UTF8 -LiteralPath $contract|ConvertFrom-Json
$required=@($policy.smoke.required_markers);$missing=@($required|Where-Object{$markerNames -notcontains $_})
