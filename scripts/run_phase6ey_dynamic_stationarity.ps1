...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6EY refuses artifact root reuse: $OutputRoot" }
New-Item -ItemType Directory -Path $OutputRoot | Out-Null
$logs = Join-Path $OutputRoot "runner-logs"
New-Item -ItemType Directory -Path $logs | Out-Null

$contractPath = Join-Path $PSScriptRoot "phase6ey_dynamic_stationarity_contract.json"
$hashPath = Join-Path $PSScriptRoot "phase6ey_dynamic_stationarity_contract.sha256"
...
if ($actualHash -ne $expectedHash) { throw "Phase 6EY contract hash mismatch" }
...
$analyzer = Join-Path $PSScriptRoot "analyze_phase6ey_dynamic_stationarity.py"
$fixture = Join-Path $PSScriptRoot "run_phase6ey_synthetic_fixture.py"
...
$reportPath = Join-Path $OutputRoot "dynamic_stationarity_report.json"
...
function Write-State([string]$Status, [int]$Completed, [string]$Active, [string]$Reason) {
    ...
    $state = [ordered]@{
        schema="campfire.phase6ey.incremental-state.v1"; phase="phase6ey"; status=$Status
...
function Invoke-Case([string]$Label, [string]$Relative, [string]$Prefix, [int]$RunIndex, [string]$Mode, [string]$ReadbackFrames) {
    ...
    $arguments = @(
        ...
        "-Collision", "true", "-Policy", "allow_self_center", "-ReportPhase", "phase6ey",
    ...
    if (-not $case.dynamic_stationarity_pass) { return "dynamic_stationarity_gate_failed" }
    if ($case.aligned_active_resource_sample_count -lt [int]$window.minimum_aligned_active_resource_samples) { return "aligned_sample_minimum_failed" }
    if ($case.outer_resource_sample_count -lt [int]$window.minimum_outer_resource_samples) { return "outer_resource_sample_minimum_failed" }
    if ($Mode -eq "acquire_discard" -and $null -eq $case.acquire_private_bytes.before) { return "acquire_markers_missing" }
    return $null
}

$completed = 0
Write-State "running" $completed "synthetic_fixture" ""
& python $fixture --contract $contractPath --output (Join-Path $OutputRoot "synthetic-fixture")
if ($LASTEXITCODE -ne 0) { Stop-Safely $completed "synthetic_fixture" "synthetic_fixture_failed" }
...
Write-Host "Phase 6EY completed: dynamic-stationarity synthetic fixture, R0 3/3, and one bounded R1 acquisition qualified"
