...
$benchmark = Join-Path $PSScriptRoot "benchmark_resident_stage_orchestrator.py"
$analyzer = Join-Path $PSScriptRoot "analyze_resident_stage_orchestrator.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6cf" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$output = Join-Path $OutputDir "resident_stage_orchestrator_raw.json"
...
& $kit @(
    ...
    "--/phase6cf/nativeLibrary=$nativeDll",
    "--/phase6cf/output=$output",
    "--/phase6cf/videoFrames=$videoFrames",
...
if ($result.status -ne "ok") { throw "Phase 6CF failed: $output" }
$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_stage_orchestrator_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_stage_orchestrator_report.svg"
$poster = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_stage_orchestrator_frame.png"
$video = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_stage_orchestrator.mp4"
& $kitPython $analyzer --raw $output --report $report --svg $svg --poster $poster --frames $videoFrames
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$ffmpeg = Get-Command ffmpeg.exe -ErrorAction SilentlyContinue
if (-not $ffmpeg) { throw "ffmpeg.exe is required to encode the Phase 6CF video." }
...
Write-Host ("Phase 6CF qualified: gates={0}, active blocks={1}, video frames={2}" -f $gateCount, $result.flow.active_blocks_peak, $result.flow.video_frame_count)
