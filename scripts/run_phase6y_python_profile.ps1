...
for ($run = 1; $run -le $RunCount; $run++) {
    ...
    & $phase3Runner -OutputDir $runOutput -ArrayBackend python -ProfileWoodInternals -PythonSurfaceBoundaryPath original -PythonStateClampPath original -CellPhaseUpdates eager -RuntimeMetrics full -RuntimeTopology dynamic
