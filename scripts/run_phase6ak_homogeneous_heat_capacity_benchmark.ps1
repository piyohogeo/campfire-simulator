...
$comparator = Join-Path $PSScriptRoot "compare_phase3_homogeneous_heat_capacity.py"
$kitPython = Join-Path $repositoryRoot "_build\windows-x86_64\release\kit\python\python.exe"

if (-not $OutputDir) {
    $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6ak\formal"
}
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$originalSummaries = @()
$homogeneousSummaries = @()
$commonArguments = @{
    ArrayBackend = "python"
...
for ($pair = 1; $pair -le $PairCount; $pair++) {
    $paths = if ($pair % 2 -eq 1) {
        ...
        @("homogeneous", "original")
    }
    foreach ($pathName in $paths) {
        $runOutput = Join-Path $OutputDir ("pair_{0}_{1}" -f $pair, $pathName)
        $homogeneousPath = if ($pathName -eq "homogeneous") { "fast" } else { "original" }
        & $phase3Runner -OutputDir $runOutput -HomogeneousHeatCapacityPath $homogeneousPath @commonArguments
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        $summary = Join-Path $runOutput "summary.json"
        if ($pathName -eq "homogeneous") {
            $homogeneousSummaries += $summary
        }
        else {
...
$compareArguments = @("--original-summary") + $originalSummaries + @(
    "--homogeneous-summary"
) + $homogeneousSummaries
