...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6GI refuses artifact root reuse: $OutputRoot" }
$contractPath = if ([string]::IsNullOrWhiteSpace($ContractPath)) {
    Join-Path $PSScriptRoot "phase6gi_s93_channel_preflight_contract.json"
...
if ($actualHash -ne $expectedHash) { throw "Phase 6GI contract hash mismatch" }
$contract = Get-Content -Raw -Encoding UTF8 $contractPath | ConvertFrom-Json
if ($contract.schema -ne "campfire.phase6gi.s93-public-channel-preflight-contract.v1" -or $contract.phase -ne "phase6gi") {
    throw "Phase 6GI contract schema mismatch"
}
...
if ((Get-FileHash -Algorithm SHA256 -LiteralPath $candidatePath).Hash -ne [string]$contract.candidate_schema.sha256) {
    throw "Phase 6GI frozen Phase 6GH candidate schema changed"
}
...
& python (Join-Path $PSScriptRoot "phase6gi_channel_preflight_policy.py") --contract $contractPath --fixtures --output $fixturePath
if ($LASTEXITCODE -ne 0) { throw "Phase 6GI schema fixture failed before Kit" }
$fixture = Get-Content -Raw -Encoding UTF8 $fixturePath | ConvertFrom-Json
if (-not $fixture.all_pass) { throw "Phase 6GI schema fixture report is not all-pass" }
...
if ($LASTEXITCODE -ne 0) { throw "Phase 6GI startup replacement fixture failed before Kit" }
$startupFixture = Get-Content -Raw -Encoding UTF8 $startupFixturePath | ConvertFrom-Json
if ($startupFixture.status -ne "pass") { throw "Phase 6GI startup replacement fixture report is not pass" }

$plan = [ordered]@{
    schema = "campfire.phase6gi.s93-channel-preflight-plan.v1"; phase = "phase6gi"; status = "running"
...
function Stop-Preflight([string]$Reason, [string]$Classification) {
    ...
    Save-Plan
    throw "Phase 6GI safe stop: $Reason ($Classification)"
}
Save-Plan

$child = Join-Path $PSScriptRoot "run_phase6gd_channel_metadata_probe.ps1"
$probe = Join-Path $PSScriptRoot "probe_phase6gi_s93_channel_preflight.py"
$policy = Join-Path $PSScriptRoot "phase6gi_preflight_attempt_policy.py"
...
while (-not $accepted) {
    ...
    $arguments = @(
        ...
        "-ReportPhase", "phase6gi", "-ProbePath", $probe, "-RequireDiscoveryUnmapped", "false",
...
Save-Plan
$summary = [ordered]@{
    schema = "campfire.phase6gi.s93-channel-preflight-summary.v1"; phase = "phase6gi"; status = "qualified"
    ...
    fixture_total = [int]$fixture.total; total_launches = [int]$plan.total_launches
...
Write-Host "Phase 6GI S93 channel preflight qualified; formal comparison remains blocked."
