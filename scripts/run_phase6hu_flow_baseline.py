"""Run the single fixed Phase 6HU Collision-OFF visible Flow baseline."""
...
CONTRACT = ROOT / "scripts/phase6hu_atomic_flow_baseline_contract.json"
SIDECAR = ROOT / "scripts/phase6hu_atomic_flow_baseline_contract.sha256"
SCHEMA = ROOT / "scripts/phase6hs_operation_report_schema.json"
CASE = ROOT / "scripts/run_phase6hu_kit_case.ps1"
PROBE = ROOT / "scripts/probe_phase6hu_flow_baseline.py"
...
def frozen_contract() -> tuple[dict, str, str]:
    ...
    if digest != expected:
        raise RuntimeError("Phase 6HU contract digest mismatch")
    policy = json.loads(data)
    schema_digest = sha256_bytes(SCHEMA.read_bytes())
    if policy["operation_report"]["schema_sha256"] != schema_digest:
        raise RuntimeError("Phase 6HU operation schema binding mismatch")
...
def build_target(paths: dict[str, Path], attempt_id: str, contract_sha: str) -> list[str]:
    return build_powershell_target(CASE, [
        ...
        "-AttemptId", attempt_id, "-ProducerPath", str(PRODUCER), "-SchemaPath", str(SCHEMA), "-ContractSha256", contract_sha,
        "-SystemPythonPath", str(SYSTEM_PYTHON), "-StageCloseTimeoutSeconds", "180",
    ])


def validate_target(target: list[str]) -> tuple[bool, str]:
    checks = (
        (target[0], POWERSHELL, "root"), (target[target.index("-KitPath") + 1], KIT, "kit"),
        (target[target.index("-AppPath") + 1], APP, "app"), (target[target.index("-ProbePath") + 1], PROBE, "probe"),
        (target[target.index("-ProducerPath") + 1], PRODUCER, "producer"), (target[target.index("-SystemPythonPath") + 1], SYSTEM_PYTHON, "python"),
    )
...
def functional_gates(operation: dict, policy: dict) -> tuple[dict, dict]:
    ...
    required = (
        "baseline_digest_unchanged_after_proxy_exclusion", "only_proxy_prim_added", "proxy_mesh_type", "proxy_collision_api",
    ...
    minimum = policy["runtime"]["active_blocks_each_sample_minimum"]
    gates = {
        "phase_and_condition": raw.get("diagnostic_phase") == "phase6hu" and raw.get("condition") == "collision_off",
        "collision_disabled": raw.get("collision_enabled") is False,
        ...
        "source_contract": runtime.get("source_center_m") == [0.0, 0.0, 0.55] and runtime.get("source_radius_m") == 0.2,
...
def run_baseline(root: Path, policy: dict, contract_sha: str, schema_sha: str) -> dict:
    attempt = root / "collision_off"
    attempt.mkdir()
    paths = paths_for(attempt)
    attempt_id = "phase6hu-collision-off-attempt01"
    target = build_target(paths, attempt_id, contract_sha)
    target_ok, target_reason = validate_target(target)
    write_json(attempt / "launch_contract.json", {"schema": "campfire.phase6hu.launch.v1", "attempt_id": attempt_id, "condition": "collision_off", "target": target, "validation": [target_ok, target_reason], "cwd": str(ROOT)})
    if not target_ok:
    ...
    validation, operation, _, _ = validate_paths(
        paths["output"], paths["markers"], expected_attempt_id=attempt_id,
        expected_schema_sha256=schema_sha, expected_contract_sha256=contract_sha,
    ) if paths["output"].is_file() else ({"accepted": False, "reason": "report_missing"}, {}, [], b"")
    ...
    gates, raw = functional_gates(operation, policy)
    ...
    result = {
        "schema": "campfire.phase6hu.condition-summary.v1", "condition": "collision_off", "status": "qualified" if passed else "safe_stop",
        ...
        "case_status": None if case is None else case.get("status"), "runner_evidence": case,
    }
...
def main() -> int:
    ...
    if root.exists():
        raise RuntimeError("Phase 6HU refuses artifact root reuse")
    ...
    if json.loads(args.preflight_report.read_text(encoding="utf-8")).get("status") != "qualified":
        raise RuntimeError("Phase 6HU no-Kit preflight missing")
    ...
    before = hashes()
    condition = run_baseline(root, policy, contract_sha, schema_sha)
    after = hashes()
    ...
    summary = {
        "schema": "campfire.phase6hu.summary.v1", "phase": "phase6hu", "status": status,
        "contract_sha256": contract_sha, "condition": condition, "visual_evidence": visual,
        "invariant_hashes_before": before, "invariant_hashes_after": after, "invariants_pass": invariant,
        "phase6ht_status_preserved": "safe_stop", "phase6ht_reclassified": False, "phase6ht_runtime_reused": False,
        "production_changed": False, "defaults_changed": False, "point_policy_changed": False, "v3_changed": False, "latest_demo_changed": False,
        "collision_on_started": False, "occlusion_comparison_started": False,
    }
