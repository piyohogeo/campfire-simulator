...
$benchmark = Join-Path $PSScriptRoot "benchmark_native_publish_boundary.py"
$analyzer = Join-Path $PSScriptRoot "analyze_native_publish_boundary.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6ay" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$rawReport = Join-Path $OutputDir "native_publish_boundary_raw.json"
...
& $kitPython $benchmark --dll $nativeDll --logs 20 --iterations $Iterations --runs $RunCount --output $rawReport
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
