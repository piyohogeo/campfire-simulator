"""Analyze Phase 6FY memory validity, lifecycle, and diagnostic cleanup separately."""
...
from phase6fy_three_axis_policy import MEMORY_VALID, classify_attempt, evaluate_population
...
def _base_report(root: Path, contract: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="phase6fy-base-") as directory:
...
def build(root: Path, contract_path: Path) -> dict:
    ...
    if not contract or contract.get("phase") != "phase6fy":
        raise ValueError("invalid Phase 6FY contract")
    ...
    return {
        "schema": "campfire.phase6fy.three-axis-memory-qualification-report.v1",
        "phase": "phase6fy",
