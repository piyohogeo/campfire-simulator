...
# Preserve the lexical build spelling; never Resolve-Path these deployment roots.
...
$arguments=@($app,"--no-window","--/app/file/ignoreUnsavedOnExit=true","--/app/fastShutdown=0","--/app/quitAfter=300000",
 ...
 "--/phase6hp/output=$output","--/phase6hp/markers=$markers","--/phase6hp/stageCloseTimeoutSeconds=$StageCloseTimeoutSeconds",
 "--/phase6hp/expectedCampfirePath=$ExpectedCampfirePath","--/phase6hp/expectedAnimPath=$ExpectedAnimPath",
...
$evidence=[ordered]@{schema="campfire.phase6hp.kit-case-runner.v1";phase="phase6hp";mode=$Mode;status=if($qualified){"qualified"}else{"failed"};failure=$failure
