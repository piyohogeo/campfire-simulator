...
if (Test-Path -LiteralPath $output) { throw "Phase 6FZ refuses case output reuse: $output" }
...
$importAuditPath = Join-Path $output "kit_import_audit.json"
$probe = Join-Path $PSScriptRoot "probe_phase6fz_three_axis_memory.py"
...
$arguments = @(
    ...
    "--/phase6fz/measurementCommitAck=$measurementAck",
    "--/phase6fz/measurementCommitFailure=$measurementFailure",
    "--/phase6fz/measurementCommitTimeoutSeconds=60",
...
$exitMarker = [ordered]@{
    schema="campfire.phase6fz.runner-lifecycle-marker.v1"; marker="os_process_exit_observed"
...
$evidence = [ordered]@{
    schema="campfire.phase6fz.memory-case-runner.v1"; phase="phase6fz"; condition=$Condition
    ...
    kit_import_audit=if (Test-Path -LiteralPath $importAuditPath) { Get-Content -Raw -Encoding UTF8 $importAuditPath | ConvertFrom-Json } else { $null }
...
if (-not $registryUnchanged) { throw "Phase 6FZ changed crash registry" }
if ($productionHashBefore -ne $productionHashAfter) { throw "Phase 6FZ changed production app" }
if ($dumps.Count -gt 0 -or $fatalLines.Count -gt 0 -or $uploadLines.Count -gt 0) { throw "Phase 6FZ safety evidence failed" }
if (-not (Test-Path -LiteralPath $measurementAck)) { throw "Phase 6FZ pre-close memory artifact was not committed" }
if ($null -eq $probeReport) { throw "Phase 6FZ probe report missing" }
if ($probeReport.lifecycle_marker -ne "shutdown_complete" -or $monitor.exit_code -ne 0) {
    throw "Phase 6FZ lifecycle did not exit normally"
}
Write-Host "Phase 6FZ case completed: $AttemptId $Condition"
