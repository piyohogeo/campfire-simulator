...
$benchmark = Join-Path $PSScriptRoot "benchmark_resident_surface_session.py"
$analyzer = Join-Path $PSScriptRoot "analyze_resident_surface_session.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6cd" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$output = Join-Path $OutputDir "resident_surface_session_raw.json"
...
& $kit @(
    ...
    "--/phase6cd/nativeLibrary=$nativeDll",
    "--/phase6cd/output=$output",
    "--/phase6cd/videoFrames=$videoFrames",
...
if ($result.status -ne "ok") { throw "Phase 6CD failed: $output" }
$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_surface_session_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_surface_session_report.svg"
$poster = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_surface_session_frame.png"
$video = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_surface_session.mp4"
& $kitPython $analyzer --raw $output --report $report --svg $svg --poster $poster --frames $videoFrames
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$ffmpeg = Get-Command ffmpeg.exe -ErrorAction SilentlyContinue
if (-not $ffmpeg) { throw "ffmpeg.exe is required to encode the Phase 6CD video." }
...
Write-Host ("Phase 6CD qualified: gates={0}, active blocks={1}, video frames={2}" -f $gateCount, $result.flow.active_blocks_peak, $result.flow.video_frame_count)
