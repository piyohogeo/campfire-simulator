...
$arguments=@($app,"--no-window","--/app/file/ignoreUnsavedOnExit=true","--/app/fastShutdown=0","--/app/quitAfter=300000",
 ...
 "--/phase6hs/attemptId=$AttemptId","--/phase6hv/condition=$Condition","--/phase6hv/contract=$contract",
...
$evidence=[ordered]@{schema="campfire.phase6hv.kit-case-runner.v1";phase="phase6hv";condition=$Condition;mode="proxy";status=if($qualified){"qualified"}else{"failed"};failure=$failure
