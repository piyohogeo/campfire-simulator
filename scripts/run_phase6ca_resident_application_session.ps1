...
$benchmark = Join-Path $PSScriptRoot "benchmark_resident_application_session.py"
$analyzer = Join-Path $PSScriptRoot "analyze_resident_application_session.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6ca" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$raw = Join-Path $OutputDir "resident_application_session_raw.json"
...
& $kit @(
    ...
    "--/phase6ca/nativeLibrary=$nativeDll",
    "--/phase6ca/output=$raw",
    "--exec",
    $benchmark
)
...
$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_application_session_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_application_session_report.svg"
