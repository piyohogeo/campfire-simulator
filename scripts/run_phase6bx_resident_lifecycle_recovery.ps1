...
$benchmark = Join-Path $PSScriptRoot "benchmark_resident_lifecycle_recovery.py"
$analyzer = Join-Path $PSScriptRoot "analyze_resident_lifecycle_recovery.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6bx" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$raw = Join-Path $OutputDir "resident_lifecycle_recovery_raw.json"
...
& $kit @(
    ...
    "--/phase6bx/nativeLibrary=$nativeDll",
    "--/phase6bx/output=$raw",
    "--exec",
    $benchmark
)
...
$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_lifecycle_recovery_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_lifecycle_recovery_report.svg"
