...
if (-not $OriginalProfileSummary -and -not $FastProfileSummary) {
    ...
    & $phase3Runner -OutputDir $originalProfileOutput -ArrayBackend python -ProfileWoodInternals -PythonSurfaceBoundaryPath fast -PythonStateClampPath original -CellPhaseUpdates eager
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
    }
    & $phase3Runner -OutputDir $fastProfileOutput -ArrayBackend python -ProfileWoodInternals -PythonSurfaceBoundaryPath fast -PythonStateClampPath fast -CellPhaseUpdates eager
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
    }
...
for ($pair = 1; $pair -le $PairCount; $pair++) {
    ...
    foreach ($pathName in $paths) {
        ...
        & $phase3Runner -OutputDir $runOutput -ArrayBackend python -PythonSurfaceBoundaryPath fast -PythonStateClampPath $pathName -CellPhaseUpdates eager
        if ($LASTEXITCODE -ne 0) {
            exit $LASTEXITCODE
        }
