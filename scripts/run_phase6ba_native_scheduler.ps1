...
$benchmark = Join-Path $PSScriptRoot "benchmark_native_scheduler_contract.py"
$analyzer = Join-Path $PSScriptRoot "analyze_native_scheduler_contract.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6ba" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$rawReport = Join-Path $OutputDir "native_scheduler_contract_raw.json"
