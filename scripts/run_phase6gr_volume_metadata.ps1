...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6GR refuses artifact root reuse: $OutputRoot" }
$contractPath = if ($ContractPath) { [IO.Path]::GetFullPath($ContractPath) } else { Join-Path $PSScriptRoot "phase6gr_volume_metadata_contract.json" }
...
if ($actualHash -ne $expectedHash) { throw "Phase 6GR contract hash mismatch" }
...
& python (Join-Path $PSScriptRoot "test_phase6gr_volume_metadata.py") *> (Join-Path $preflight "fixture.log")
if ($LASTEXITCODE -ne 0) { throw "Phase 6GR offline fixture failed before Kit launch" }
[IO.File]::WriteAllText((Join-Path $preflight "result.json"),'{"schema":"campfire.phase6gr.offline-fixture.v1","passed":true,"case_count":26}'+[Environment]::NewLine,[Text.UTF8Encoding]::new($false))
...
$state = [ordered]@{
    schema="campfire.phase6gr.incremental-state.v1";status="running";active_condition="temperature_volume_metadata_once";
...
$probe = Join-Path $PSScriptRoot "probe_phase6gr_volume_metadata.py"
...
$arguments = @(
    ...
    "-Filtering","true","-Collision","true","-Policy","allow_self_center","-ReportPhase","phase6gr",
...
$accessorsComplete = ($null -ne $operation -and
...
$operationComplete = ($null -ne $operation -and $operation.status -eq "pass" -and
    ...
    $operation.volume_conversion_calls -eq 1 -and $operation.bounded_metadata_complete -and
    $accessorsComplete -and $operation.last_operation_marker -eq "phase6gr_source_release_after" -and
...
$summary = [ordered]@{
    schema="campfire.phase6gr.volume-metadata-summary.v1";qualified=[bool]$qualified;
...
[IO.File]::WriteAllText((Join-Path $OutputRoot "phase6gr_summary.json"),($summary|ConvertTo-Json -Depth 16)+[Environment]::NewLine,[Text.UTF8Encoding]::new($false))
...
$state.last_successful_accessor=if($operation){$operation.last_successful_accessor}else{$null}
[IO.File]::WriteAllText($statePath,($state|ConvertTo-Json -Depth 8)+[Environment]::NewLine,[Text.UTF8Encoding]::new($false))
if (-not $qualified) { throw "Phase 6GR safe stop: operation=$operationResult lifecycle=$lifecycleResult" }
