...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6FM refuses artifact root reuse: $OutputRoot" }
$contractPath = if ([string]::IsNullOrWhiteSpace($ContractPath)) {
    Join-Path $PSScriptRoot "phase6fm_settled_three_iteration_contract.json"
...
if ($actualHash -ne $expectedHash) { throw "Phase 6FM contract hash mismatch" }
...
foreach ($item in $contract.runtime_implementation) {
    ...
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $path).Hash -ne [string]$item.sha256) {
        throw "Phase 6FM runtime implementation hash changed: $($item.path)"
    }
}
...
if ((Get-FileHash -Algorithm SHA256 -LiteralPath $basePath).Hash -ne $contract.base_operation_contract.sha256) {
    throw "Phase 6FM base operation contract changed"
}
...
$analyzer = Join-Path $PSScriptRoot "analyze_phase6fm_settled_three_iteration.py"
...
function Write-State([string]$Status, [string]$ActiveAttempt, [string]$ActiveSlot, [string]$Classification, [string]$Reason) {
    $payload = [ordered]@{
        schema = "campfire.phase6fm.incremental-state.v1"; phase = "phase6fm"; status = $Status
...
function Update-Report {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 6FM analyzer failed" }
}
function Assert-Production([string]$Boundary) {
    if ((Get-FileHash -Algorithm SHA256 -LiteralPath $productionApp).Hash -ne $productionBefore) {
        ...
        throw "Phase 6FM production app changed"
    }
}
...
while ($slotIndex -lt $slots.Count -and $attempted -lt [int]$contract.population.maximum_launches) {
    ...
    $metadata = [ordered]@{
        schema = "campfire.phase6fm.attempt-metadata.v1"; attempt_id = $attemptId
    ...
    try { Update-Report } catch {
        ...
        Write-Error "Phase 6FM analyzer failed after $attemptId"; exit 2
    }
    ...
    if ($classification -eq "representative_pass") {
        if ($null -ne $report.first_replicated_settled_failure) {
            $reason = [string]$report.first_replicated_settled_failure.failure
            Write-State "operation_safe_stop" $attemptId $slot.slot_id "operation_failure" $reason
            Write-Error "Phase 6FM captured replicated settled accumulation failure; later slots were not started"; exit 2
        }
    ...
    $status = if ($classification -eq "operation_failure") { "operation_safe_stop" } elseif ($classification -eq "native_lifecycle_failure") { "native_lifecycle_safe_stop" } else { "absolute_safety_stop" }
    ...
    Write-Error "Phase 6FM captured nonreplaceable $classification at $attemptId; later slots were not started"; exit 2
}
...
Write-Host "Phase 6FM completed nine representative settled-baseline processes; iteration count above three remains excluded"
