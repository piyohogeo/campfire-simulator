...
ROOT = SCRIPTS.parent
CONTRACT = SCRIPTS / "phase6ij_stage_open_composition_contract.json"
SIDECAR = SCRIPTS / "phase6ij_stage_open_composition_contract.sha256"
...
loader = _bootstrap(ROOT / policy["bootstrap_sources"]["dependency_loader"]["path"], policy["bootstrap_sources"]["dependency_loader"]["sha256"], "phase6ij_dependency_loader_runtime")
markers = _bootstrap(ROOT / policy["bootstrap_sources"]["marker_contract"]["path"], policy["bootstrap_sources"]["marker_contract"]["sha256"], "phase6ij_marker_contract_runtime")
settings = carb.settings.get_settings()
marker_path = Path(settings.get_as_string("/phase6ij/markers")).resolve()
report_path = Path(settings.get_as_string("/phase6ij/report")).resolve()
identity_path = Path(settings.get_as_string("/phase6ij/identity")).resolve()
stage_root = Path(settings.get_as_string("/phase6ij/stageRoot")).resolve()
attempt_id = settings.get_as_string("/phase6ij/attemptId")
condition = settings.get_as_string("/phase6ij/condition")
