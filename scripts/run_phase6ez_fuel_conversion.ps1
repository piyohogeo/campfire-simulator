...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6EZ refuses artifact root reuse: $OutputRoot" }
New-Item -ItemType Directory -Path $OutputRoot | Out-Null
$logs = Join-Path $OutputRoot "runner-logs"
New-Item -ItemType Directory -Path $logs | Out-Null

$contractPath = Join-Path $PSScriptRoot "phase6ez_fuel_conversion_contract.json"
$hashPath = Join-Path $PSScriptRoot "phase6ez_fuel_conversion_contract.sha256"
...
if ($actualHash -ne $expectedHash) { throw "Phase 6EZ contract hash mismatch" }
...
$reportPath = Join-Path $OutputRoot "fuel_conversion_report.json"
...
function Write-State([string]$Status, [int]$Completed, [string]$Active, [string]$Reason) {
    $payload = [ordered]@{
        schema="campfire.phase6ez.incremental-state.v1"; phase="phase6ez"; status=$Status
...
function Invoke-Case([string]$Label, [string]$Mode) {
    ...
    $arguments = @(
        ...
        "-Collision", "true", "-Policy", "allow_self_center", "-ReportPhase", "phase6ez",
...
Write-State "running" $completed "C1_fuel_convert" ""
$reason = Invoke-Case "C1_fuel_convert" "fuel_convert_release"
if ($reason) { Stop-Safely $completed "C1_fuel_convert" $reason }
$completed++
...
Write-Host "Phase 6EZ completed: C0 acquire/discard and one C1 fuel conversion qualified"
