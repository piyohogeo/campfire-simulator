...
if(Test-Path $OutputDir){throw "Phase V3T-L refuses to reuse output: $OutputDir"}
New-Item -ItemType Directory -Path $OutputDir|Out-Null
$allConditions=@(
    "ground_stones_no_lights","ground_stones_lit","cylinder20_solid",
    "v3mesh20_static_texture","flow_volume"
)
$heldPresets=@("ao_off","ao_1spp")
$allPresets=@("baseline","balanced","performance","max_bounces_2","cache_on","direct_spp_1","dome_samples_1","translucency_off","sss_off","specular_bounces_1","minimal","candidate_balanced","candidate_performance")
if(-not$Conditions.Count){$Conditions=$allConditions}
if(-not$Presets.Count){$Presets=@("baseline")}
foreach($condition in $Conditions){if($condition-notin$allConditions){throw "Invalid Phase V3T-L condition: $condition"}}
foreach($preset in $Presets){
    if($preset-in$heldPresets){throw "Phase V3T-L AO presets are held after the native crash and must not be rerun: $preset"}
    if($preset-notin$allPresets){throw "Invalid Phase V3T-L preset: $preset"}
}
...
function Assert-NoKit {
    ...
    if($old.Count){throw "Phase V3T-L refuses to overlap isolated Kit: $($old.ProcessId -join ',')"}
}
...
function Invoke-IsolatedRun([string]$Condition,[string]$Preset,[int]$Run) {
    Assert-NoKit
    $name="{0}_preset-{1}_r{2}" -f $Condition,$Preset,($Run+1)
    ...
    try{
        $quitAfterMs=[int][Math]::Ceiling(($WarmupSeconds+$MeasureSeconds+360)*1000)
        $args=@(
            ...
            if($reader){while(-not$reader.EndOfStream){
                ...
                foreach($token in $fatalTokens){
                    if($line.Contains($token)){
                        ...
                        if($classification-eq"native_crash_log"){Start-Sleep -Seconds 5}
        ...
        if(-not$classification){
            ...
            $classification=if($exitCode-eq0){"normal"}else{"native_crash_nonzero_exit"}
        }
    } finally {
    ...
    $crashEvidence=[ordered]@{
        crash_reporter_started_count=if(Test-Path $log){(Select-String -LiteralPath $log -SimpleMatch "carb.crashreporter").Count}else{0}
        dump_written_count=if(Test-Path $log){(Select-String -LiteralPath $log -SimpleMatch "Wrote dump file").Count}else{0}
        crash_reporter_dump_path_lines=@(if(Test-Path $log){Select-String -LiteralPath $log -SimpleMatch ".dmp.zip"|ForEach-Object{$_.Line}}else{@()})
    }
    if($fatal["[crash] A crash has occurred"]-gt0){$classification="native_crash_log"}
    elseif($crashEvidence.dump_written_count-gt0-or$crashEvidence.crash_reporter_dump_path_lines.Count-gt0){$classification="native_crash_reporter_dump"}
    elseif($crashEvidence.crash_reporter_started_count-gt0-and$classification-ne"normal"){$classification="native_crash_reporter"}
    if($classification-ne"normal"-or($fatal.Values|Measure-Object -Sum).Sum-ne0){throw "Phase V3T-L rejected $name ($classification): $($fatal|ConvertTo-Json -Compress)"}
    $payload=Get-Content -Raw -Encoding UTF8 $raw|ConvertFrom-Json
    if($payload.status-ne"ok"){throw "Phase V3T-L probe failed: $($payload.error)"}
    ...
    if($frameDelta-le0){throw "Phase V3T-L visible viewport produced no frames: $name"}
    ...
    $record=[ordered]@{
        name=$name;condition=$Condition;preset=$Preset;run=$Run+1;classification=$classification;exit_code=$exitCode
...
foreach($condition in $Conditions){foreach($preset in $Presets){$jobs.Add([pscustomobject]@{condition=$condition;preset=$preset})}}
if(-not$jobs.Count){throw "Phase V3T-L has no jobs"}
...
for($run=0;$run-lt$Runs;$run++){
    ...
    foreach($job in $ordered){$entries.Add((Invoke-IsolatedRun $job.condition $job.preset $run))}
}
$gpuInventory=$null
if($nvidiaSmi){
...
$manifest=[ordered]@{
    schema="campfire.phasev3tl.lightweight-rtx-manifest.v1";phase="V3T-L";mode=$Mode;runs=$Runs
    warmup_seconds=$WarmupSeconds;measure_seconds=$MeasureSeconds;conditions=$Conditions;presets=$Presets;gpu_inventory=$gpuInventory
...
Write-Host "Phase V3T-L $Mode complete: $manifestPath"
