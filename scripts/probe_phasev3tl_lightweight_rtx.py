"""Production-neutral lightweight RTX preset probe for Phase V3T-L.
...
PRESETS = {
    "baseline": {},
    "balanced": {"/rtx/post/aa/op": 3, "/rtx/post/dlss/execMode": 1},
    "performance": {"/rtx/post/aa/op": 3, "/rtx/post/dlss/execMode": 0},
    "ao_off": {"/rtx/ambientOcclusion/enabled": False},
    "ao_1spp": {"/rtx/ambientOcclusion/enabled": True, "/rtx/ambientOcclusion/minSamples": 1, "/rtx/ambientOcclusion/maxSamples": 1},
    "max_bounces_2": {"/rtx/rtpt/maxBounces": 2},
    "cache_on": {"/rtx/rtpt/cached/enabled": True, "/rtx/rtpt/lightcache/cached/enabled": True},
    "direct_spp_1": {"/rtx/directLighting/sampledLighting/samplesPerPixel": 1},
    "dome_samples_1": {"/rtx/directLighting/domeLight/sampleCount": 1},
    "translucency_off": {"/rtx/translucency/enabled": False},
    "sss_off": {"/rtx/raytracing/subsurface/enabled": False},
    "specular_bounces_1": {"/rtx/rtpt/maxSpecularAndTransmissionBounces": 1, "/rtx/translucency/maxRefractionBounces": 1},
    "minimal": {"/rtx/rendermode": "MinimalRendering", "/rtx/minimal/mode": 3},
    "candidate_balanced": {
        "/rtx/post/aa/op": 3, "/rtx/post/dlss/execMode": 1,
        "/rtx/rtpt/maxBounces": 2,
    },
    "candidate_performance": {
        "/rtx/post/aa/op": 3, "/rtx/post/dlss/execMode": 0,
        "/rtx/rtpt/maxBounces": 2,
    },
}
SETTING_PATHS = (
...
def _arguments():
    ...
    return {
        "output": Path(settings.get_as_string("/phasev3tl/output")).resolve(),
        "condition": settings.get_as_string("/phasev3tl/condition"),
        "preset": settings.get_as_string("/phasev3tl/preset"),
        "warmup_seconds": settings.get_as_float("/phasev3tl/warmupSeconds"),
        "measure_seconds": settings.get_as_float("/phasev3tl/measureSeconds"),
        "run": settings.get_as_int("/phasev3tl/run"),
    }
...
def _set_typed(settings, path, value):
...
def _apply_effective_settings(condition, preset):
    settings = carb.settings.get_settings()
    ...
    for path, value in PRESETS[preset].items():
        _set_typed(settings, path, value)
    return flow_enabled
...
async def _run(arguments):
    ...
    report = {"schema": "campfire.phasev3tl.lightweight-rtx-process.v1", "status": "error"}
    try:
        allowed = {
        ...
        if condition not in allowed or preset not in PRESETS:
            raise ValueError(f"invalid condition/preset: {condition}/{preset}")
        ...
        if not stage.GetRootLayer().Save():
            raise RuntimeError("unable to save Phase V3T-L stage")
        flow_enabled = _apply_effective_settings(condition, preset)
        ...
        _apply_effective_settings(condition, preset)
        ...
        readiness_updates = 0
        readiness_deadline = readiness_started + 180.0
        previous_frame = -1
        consecutive_increments = 0
        while time.perf_counter() < readiness_deadline:
            await app.next_update_async()
            readiness_updates += 1
            frame = _frame_snapshot(viewport)["frame_number"]
            if frame > previous_frame and previous_frame >= 0:
                consecutive_increments += 1
            elif frame <= previous_frame:
                consecutive_increments = 0
            previous_frame = frame
            if consecutive_increments >= 8:
                break
        readiness = {
            "wall_seconds": time.perf_counter() - readiness_started,
            "kit_update_count": readiness_updates,
            "consecutive_frame_increments": consecutive_increments,
            "frame_info": _frame_snapshot(viewport),
        }
        if consecutive_increments < 8:
            raise RuntimeError("visible viewport did not sustain eight consecutive frame increments within 180 seconds")
        ...
        for path, expected in PRESETS[preset].items():
            actual = settings_before["requested_paths"].get(path)
            if actual != expected:
                raise RuntimeError(f"preset effective-value mismatch: {path} requested={expected!r} actual={actual!r}")
        ...
        report = {
            "schema": "campfire.phasev3tl.lightweight-rtx-process.v1", "status": "ok",
            "condition": condition, "preset": preset, "run": arguments["run"] + 1,
        ...
        report = {"schema": "campfire.phasev3tl.lightweight-rtx-process.v1", "status": "error", "condition": arguments.get("condition"), "preset": arguments.get("preset"), "error": f"{type(error).__name__}: {error}"}
    finally:
        timeline.stop()
