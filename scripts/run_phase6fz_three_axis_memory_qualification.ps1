...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6FZ refuses artifact root reuse: $OutputRoot" }
$contractPath = if ([string]::IsNullOrWhiteSpace($ContractPath)) {
    Join-Path $PSScriptRoot "phase6fz_three_axis_memory_qualification_contract.json"
...
if ($actualHash -ne $expectedHash) { throw "Phase 6FZ contract hash mismatch" }
$contract = Get-Content -Raw -Encoding UTF8 $contractPath | ConvertFrom-Json
if ($contract.phase -ne "phase6fz") { throw "Phase 6FZ contract phase mismatch" }
$runtimeFiles = [ordered]@{
    ...
    three_axis_policy_sha256 = Join-Path $PSScriptRoot "phase6fz_three_axis_policy.py"
    preclose_committer_sha256 = Join-Path $PSScriptRoot "phase6fz_preclose_committer.py"
    analyzer_sha256 = Join-Path $PSScriptRoot "analyze_phase6fz_three_axis_memory_qualification.py"
    qualification_runner_sha256 = $PSCommandPath
    synchronized_probe_sha256 = Join-Path $PSScriptRoot "probe_phase6fz_three_axis_memory.py"
    phase6fz_case_runner_sha256 = Join-Path $PSScriptRoot "run_phase6fz_memory_case.ps1"
    fixture_runner_sha256 = Join-Path $PSScriptRoot "run_phase6fz_three_axis_fixtures.py"
    import_contract_sha256 = Join-Path $PSScriptRoot "phase6fz_import_contract.py"
    import_smoke_probe_sha256 = Join-Path $PSScriptRoot "probe_phase6fz_import_smoke.py"
    import_smoke_runner_sha256 = Join-Path $PSScriptRoot "run_phase6fz_import_smoke.ps1"
    cdb_progress_fixture_sha256 = Join-Path $PSScriptRoot "run_phase6fz_cdb_progress_fixtures.ps1"
    cdb_progress_helper_sha256 = Join-Path $PSScriptRoot "phase6fz_progress_fixture.ps1"
    phase6ea_common_sha256 = Join-Path $PSScriptRoot "phase6ea_diagnostic_common.ps1"
}
foreach ($entry in $runtimeFiles.GetEnumerator()) {
    ...
    if ($required -ne $observed) { throw "Phase 6FZ runtime hash mismatch: $($entry.Key)" }
}
...
& (Join-Path $PSScriptRoot "run_phase6fz_cdb_progress_fixtures.ps1") -OutputRoot $cdbFixtureRoot
$cdbFixtureReport = Get-Content -Raw -Encoding UTF8 (Join-Path $cdbFixtureRoot "cdb_progress_fixture_report.json") | ConvertFrom-Json
if (-not $cdbFixtureReport.passed -or $cdbFixtureReport.residual.cdb -ne 0) { throw "Phase 6FZ CDB progress fixture failed; formal population was not started" }
$fixtureRunner = Join-Path $PSScriptRoot "run_phase6fz_three_axis_fixtures.py"
$fixtureRoot = Join-Path $OutputRoot "fixtures"
& $python $fixtureRunner --contract $contractPath --output-root $fixtureRoot
if ($LASTEXITCODE -ne 0) { throw "Phase 6FZ fixture gate failed; real Kit was not started" }
$fixtureReport = Get-Content -Raw -Encoding UTF8 (Join-Path $fixtureRoot "fixture_report.json") | ConvertFrom-Json
if (-not $fixtureReport.passed -or $fixtureReport.total_count -ne 20) { throw "Phase 6FZ fixture report failed closed" }

$guard = Join-Path $PSScriptRoot "phase6fu_resource_guard.py"
$caseRunner = Join-Path $PSScriptRoot "run_phase6fz_memory_case.ps1"
$committer = Join-Path $PSScriptRoot "phase6fz_preclose_committer.py"
$analyzer = Join-Path $PSScriptRoot "analyze_phase6fz_three_axis_memory_qualification.py"
...
function Write-State([string]$Status, [string]$AttemptId, [string]$Classification, [string]$Reason) {
    $state = [ordered]@{
        schema="campfire.phase6fz.incremental-state.v1"; phase="phase6fz"; status=$Status
        ...
        fixture_passed=$true; import_smoke_passed=$true; cdb_progress_fixture_passed=$true; contract_sha256=$actualHash; production_sha256=$productionBefore
...
function Update-Report {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 6FZ analyzer failed" }
}
...
function Assert-Production([string]$Boundary) {
    ...
    if ($current -ne $productionBefore) {
        ...
        throw "Phase 6FZ production app hash changed"
    }
}
...
function Invoke-Attempt([object]$Slot, [string]$SlotKind, [string]$ReplacementFor) {
    ...
    if ($launched -gt [int]$contract.population.maximum_total_launches) { throw "Phase 6FZ total launch limit exceeded" }
    ...
    $metadata = [ordered]@{
        schema="campfire.phase6fz.attempt-metadata.v1"; phase="phase6fz"; attempt_id=$attemptId
    ...
    if (-not $committerProcess.WaitForExit(15000)) {
        ...
        throw "Phase 6FZ committer child did not exit"
    }
    ...
    if ($row.classification -eq "memory_valid_lifecycle_timeout") {
        ...
        throw "Phase 6FZ nonreplaceable attempt: $($row.failures -join ',')"
    }
    if ($report.population.population_stopping_failure) {
        ...
        throw "Phase 6FZ population policy stopped: $($report.population.failures -join ',')"
    }
}
...
foreach ($item in @($replacementQueue)) {
    if ($replacementLaunched -ge [int]$contract.population.maximum_timeout_replacements) {
        ...
        throw "Phase 6FZ replacement limit exceeded"
    }
...
if (-not $final.memory_ceiling_qualified) {
    ...
    throw "Phase 6FZ memory ceiling did not qualify"
}
Write-State "qualified" "complete" "memory_qualified_lifecycle_separate" ""
Write-Host "Phase 6FZ memory ceiling qualified; lifecycle remains separate and Phase 6FO remains stopped"
