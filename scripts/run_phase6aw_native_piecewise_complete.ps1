...
$benchmark = Join-Path $PSScriptRoot "benchmark_native_piecewise_complete_step.py"
$analyzer = Join-Path $PSScriptRoot "analyze_native_piecewise_complete_step.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6aw" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$rawReport = Join-Path $OutputDir "native_piecewise_complete_step_raw.json"
