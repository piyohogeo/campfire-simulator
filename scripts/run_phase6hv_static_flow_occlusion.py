"""Run the fixed OFF/ON Phase 6HV static Flow occlusion comparison."""
...
CONTRACT = ROOT / "scripts/phase6hv_static_flow_occlusion_contract.json"
SIDECAR = ROOT / "scripts/phase6hv_static_flow_occlusion_contract.sha256"
SCHEMA = ROOT / "scripts/phase6hs_operation_report_schema.json"
CASE = ROOT / "scripts/run_phase6hv_kit_case.ps1"
PROBE = ROOT / "scripts/probe_phase6hv_static_flow_occlusion.py"
...
def frozen_contract() -> tuple[dict, str, str]:
    ...
    if digest != SIDECAR.read_text(encoding="ascii").split()[0].upper():
        raise RuntimeError("Phase 6HV contract digest mismatch")
    policy = json.loads(data)
    schema_digest = sha256_bytes(SCHEMA.read_bytes())
    if policy["operation_report"]["schema_sha256"] != schema_digest:
        raise RuntimeError("Phase 6HV operation schema binding mismatch")
...
def functional_gates(operation: dict, condition: str, policy: dict) -> tuple[dict, dict]:
    ...
    minimum = policy["flow_liveness"][f"{condition}_active_blocks_each_sample_minimum"]
    gates = {
        "phase_and_condition": raw.get("diagnostic_phase") == "phase6hv" and raw.get("condition") == condition,
        ...
        "flow_liveness": len(samples) == 4 and all(item.get("active_blocks", 0) >= minimum for item in samples),
        "source_contract": runtime.get("source_center_m") == [0.0, 0.0, 0.55] and runtime.get("source_radius_m") == 0.2,
        "stage_contract": (raw.get("stage_contract") or {}).get("passed") is True,
...
def stage_difference(root: Path) -> dict:
    ...
    expected = len(changes) == 2 and all("physicsCollisionEnabled" in line for line in changes) and any(" = 1" in line for line in changes) and any(" = 0" in line for line in changes)
    on_validation = validate_stage(on_path, frozen_contract()[0], "collision_on")
    off_validation = validate_stage(off_path, frozen_contract()[0], "collision_off")
    return {"passed": expected and on_validation["passed"] and off_validation["passed"], "changed_lines": changes, "on_path": str(on_path), "off_path": str(off_path), "on_validation": on_validation, "off_validation": off_validation}
...
def run_condition(root: Path, condition: str, policy: dict, contract_sha: str, schema_sha: str) -> dict:
    ...
    attempt_id = f"phase6hv-{condition}-attempt01"
    target = build_target(paths, attempt_id, condition, contract_sha)
    target_ok, target_reason = validate_target(target, condition)
    write_json(attempt / "launch_contract.json", {"schema": "campfire.phase6hv.launch.v1", "attempt_id": attempt_id, "condition": condition, "target": target, "validation": [target_ok, target_reason], "cwd": str(ROOT)})
    if not target_ok:
    ...
    result = {
        "schema": "campfire.phase6hv.condition-summary.v1", "condition": condition, "status": "qualified" if passed else "safe_stop",
...
def main() -> int:
    ...
    parser.add_argument("--phase6hu-summary", type=Path, required=True)
    parser.add_argument("--preflight-report", type=Path, required=True)
    args = parser.parse_args()
    root = args.artifact_root.absolute()
    if root.exists():
        raise RuntimeError("Phase 6HV refuses artifact root reuse")
    if json.loads(args.phase6hu_summary.read_text(encoding="utf-8")).get("status") != "qualified":
        raise RuntimeError("Phase 6HU qualified precondition missing")
    if json.loads(args.preflight_report.read_text(encoding="utf-8")).get("status") != "qualified":
        raise RuntimeError("Phase 6HV no-Kit preflight missing")
    ...
    for condition in [item["name"] for item in policy["condition_order"]]:
        ...
        if results[condition]["status"] != "qualified":
            write_json(root / "summary.json", {"schema": "campfire.phase6hv.summary.v1", "phase": "phase6hv", "status": "safe_stop", "conditions": results, "stopped_after": condition, "phase6hs_reclassified": False, "phase6ht_reclassified": False, "phase6hu_reclassified": False, "production_changed": False})
            return 1
    ...
    summary = {
        "schema": "campfire.phase6hv.summary.v1", "phase": "phase6hv", "status": status,
        ...
        "phase6hs_reclassified": False, "phase6ht_reclassified": False, "phase6hu_reclassified": False,
        "phase6hs_runtime_reused": False, "phase6ht_runtime_reused": False, "phase6hu_runtime_reused": False, "production_changed": False,
