...
def analyze_attempt(attempt_root: Path, contract: dict, preflight=False):
    ...
    if contract.get("phase") in {"phase6gl", "phase6gm", "phase6gn"}:
        ...
        required_imports = (
            ...
            if contract.get("phase") in {"phase6gm", "phase6gn"}
        ...
        import_ok = bool(
            ...
            == (Path(__file__).resolve().parent / ({"phase6gm": "probe_phase6gm_supply_comparison.py", "phase6gn": "probe_phase6gn_supply_comparison.py"}.get(contract.get("phase"), "probe_phase6gl_supply_comparison.py"))).resolve()
            and required_imports.issubset(resolved)
            and (contract.get("phase") != "phase6gn" or bool((audit.get("module_contract") or {}).get("pass")))
            and (contract.get("phase") != "phase6gn" or bool((audit.get("wrapper_wiring") or {}).get("patched_identity_matches")))
        )
    else:
        import_ok = bool(
            audit.get("status") == "pass"
...
def build(root: Path, contract_path: Path, offline_path: Path):
    ...
    if phase not in {"phase6ga", "phase6gb", "phase6gc", "phase6gl", "phase6gm", "phase6gn"}: raise ValueError("invalid guarded supply contract")
    ...
    inherited_preflight = phase in {"phase6gl", "phase6gm", "phase6gn"} and contract.get("public_channel_schema", {}).get("qualification") == "phase6gk_preflight_qualified"
