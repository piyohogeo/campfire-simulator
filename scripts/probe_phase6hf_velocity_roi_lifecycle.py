"""Phase 6HF cumulative velocity ROI-sampling lifecycle probe."""
...
if CONDITION not in ROW_BY_NAME:
    raise ImportError(f"unsupported Phase 6HF condition: {CONDITION!r}")
ROW = ROW_BY_NAME[CONDITION]

hb, hb_audit = load_exact_module(
    HB_PATH, HB_PATH, module_name="campfire_phase6hf_phase6hb_base", required_entrypoints=()
)
hb.report = new_runtime_report(condition=CONDITION, attempt_id=CONDITION)


def checkpoint(name: str, **values) -> None:
    append_checkpoint(hb.report, name, **values)
...
def append_resource_marker(path, name, *args, **kwargs):
    translated = name.replace("phase6hb_", "phase6hf_", 1) if name.startswith("phase6hb_") else name
    if kwargs.get("phase") == "phase6hb":
        kwargs["phase"] = "phase6hf"
...
def velocity_boundary(flow, volume_provider, arguments, frame, output, collectors, operation_state=None):
    ...
    def marker(name: str, **values) -> None:
        canonical = name if name.startswith("phase6hf_") else f"phase6hf_{name}"
    ...
    volume_row = None
    grid = None
    metadata_root = output.parent / f"phase6hf-schema-f{int(frame):04d}"
    ...
    try:
        ...
        for index, source in enumerate(handles):
            ...
            increment_counter(hb.report, "array_metadata")
        ...
        for index in range(1, 6):
            ...
            for counter in (
                "schema_volume_conversion", "schema_metadata",
                "schema_temporary_save", "schema_typed_read",
            ):
                increment_counter(hb.report, counter)
            volume_row, file_bytes = hb.gz._instrumented_schema_volume(
                flow,
                volume_provider,
                handles[index],
                path,
                hb.discovery.TOTAL_FILE_LIMIT - total_bytes,
                schema_marker,
                index,
            )
        ...
        increment_counter(hb.report, "velocity_alias_metadata")
        ...
        counter_after_events = {
            ...
            "velocity_profile_after": "velocity_profile",
        ...
        marker("velocity_pipeline_before", roi_count=roi_count, roi_names=ROW["roi_names"])
        velocity_result = hb.shared._save_and_sample(
            ...
            diagnostic_roi_limit=None if roi_count == 0 else roi_count,
        )
        ...
        marker("release_before", local_result_count=len(hb.report["executed_roi_names"]))
        ...
        marker("release_after", weak_reference_alive_count=weak_alive)
        if weak_alive:
        ...
        if hb.report["executed_roi_names"] != ROW["roi_names"]:
            raise RuntimeError("executed ROI sequence does not match the frozen cumulative prefix")
        complete_operation(hb.report)
        write_operation_report(hb.REPORT_PATH, hb.report)
        return {
            "mode": f"phase6hf_{ROW['mode']}",
...
hb._atomic_json(hb.AUDIT_PATH, {
    "schema": "campfire.phase6hf.kit-import-audit.v1",
    "status": "pass",
