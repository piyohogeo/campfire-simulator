...
$benchmark = Join-Path $PSScriptRoot "benchmark_native_wood_boundary.py"
$analyzer = Join-Path $PSScriptRoot "analyze_native_wood_boundary.py"
...
if (-not $OutputDir) {
    $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6au"
}
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$rawReport = Join-Path $OutputDir "native_wood_boundary_raw.json"
