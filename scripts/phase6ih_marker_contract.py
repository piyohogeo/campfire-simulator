...
EVENT_FIELDS={
 "kit_launch":{"attempt_id":str,"executable_path":str},"kit_app_ready":{"attempt_id":str},"dependencies_complete":{"module_count":int},
 "layer_generation_started":{"condition":str},"layer_generation_complete":{"protected_sha256":str,"container_sha256":str,"runtime_sha256":str},
 "isolation_snapshot_started":{"boundary":str,"sequence_index":int},"isolation_snapshot_complete":{"boundary":str,"sequence_index":int,"snapshot_sha256":str},
 ...
 "isolation_sequence_validated":{"classification":str,"accepted":bool},"operation_complete":{"scope":str},
...
def append_marker(marker_file:Path,event_name:str,payload:Mapping[str,object])->dict:
 canonical=canonical_payload(event_name,payload);marker_file=Path(marker_file);marker_file.parent.mkdir(parents=True,exist_ok=True);row={"marker":event_name,"timestamp_utc":datetime.now(timezone.utc).isoformat(),**canonical}
...
def canonical_payload(event_name:str,payload:Mapping[str,object])->dict:
 ...
 for key,kind in expected.items():
  value=payload[key];valid=type(value)is kind if kind in(bool,int) else isinstance(value,kind)
 ...
 return dict(payload)
def produce_marker(event_name:str,**values:object):return event_name,canonical_payload(event_name,values)
