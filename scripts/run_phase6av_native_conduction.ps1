...
$benchmark = Join-Path $PSScriptRoot "benchmark_native_conduction_boundary.py"
$analyzer = Join-Path $PSScriptRoot "analyze_native_conduction_boundary.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6av" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$rawReport = Join-Path $OutputDir "native_conduction_boundary_raw.json"
