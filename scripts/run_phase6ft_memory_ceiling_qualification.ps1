...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6FT refuses artifact root reuse: $OutputRoot" }
$contractPath = if ([string]::IsNullOrWhiteSpace($ContractPath)) { Join-Path $PSScriptRoot "phase6ft_memory_ceiling_qualification_contract.json" } else { [IO.Path]::GetFullPath($ContractPath) }
...
if ($actualHash -ne $expectedHash) { throw "Phase 6FT contract hash mismatch" }
...
function Write-State([string]$Status, [string]$AttemptId, [string]$SlotId, [string]$Classification, [string]$Reason) {
    $state = [ordered]@{
        schema="campfire.phase6ft.incremental-state.v1"; phase="phase6ft"; status=$Status
        ...
        kit_hard_limit_bytes=[long]$contract.safety.kit_provisional_hard_limit_bytes
        unique_tree_hard_limit_bytes=[long]$contract.safety.unique_tree_provisional_hard_limit_bytes
        phase6fo_restarted=$false; production_shutdown_order_changed=$false
...
function Update-Report {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Phase 6FT analyzer failed" }
}
...
function Assert-Production([string]$Boundary) {
    ...
    if ($after -ne $productionBefore) {
        ...
        throw "Phase 6FT production app hash changed"
    }
}
...
function Invoke-MemoryCase([string]$AttemptRoot, [object]$Condition, [int]$RunIndex, [string]$AttemptId) {
    ...
    $arguments = @(
        ...
        "-ReportPhase", "phase6ft", "-GeometryVariant", $contract.physical_fixture.geometry_variant,
    ...
    $guardArgs = @(
        ...
        "--kit-private-limit", "$($contract.safety.kit_provisional_hard_limit_bytes)",
        "--tree-private-limit", "$($contract.safety.unique_tree_provisional_hard_limit_bytes)",
...
while ($completed -lt $slots.Count) {
    ...
    $metadata = [ordered]@{
        schema="campfire.phase6ft.attempt-metadata.v1"; phase="phase6ft"; attempt_id=$attemptId
    ...
    if ($case.classification -eq "startup_prerequisite_failure") {
        ...
        throw "Phase 6FT startup replacement budget exhausted"
    }
    Write-State "safe_stop" $attemptId $slot.slot_id $case.classification ($case.failures -join ',')
    throw "Phase 6FT nonreplaceable failure: $($case.failures -join ',')"
}

Update-Report
Assert-Production "complete"
$final = Get-Content -Raw -Encoding UTF8 $reportPath | ConvertFrom-Json
if (-not $final.qualification_complete -or -not $final.candidate_16_gib_qualified) {
    Write-State "safe_stop" "complete" "" "nonreplaceable_failure" "final_report_not_qualified"
    throw "Phase 6FT memory ceiling qualification did not qualify"
}
Write-State "qualified" "complete" "complete" "representative_pass" ""
Write-Host "Phase 6FT memory ceiling qualification completed; Phase 6FO remains stopped"
