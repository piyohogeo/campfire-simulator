...
$arguments=@($app,"--no-window","--/app/file/ignoreUnsavedOnExit=true","--/app/fastShutdown=0","--/app/quitAfter=300000",
 ...
 "--/phase6hs/attemptId=$AttemptId","--/phase6ht/condition=$Condition",
...
$evidence=[ordered]@{schema="campfire.phase6ht.kit-case-runner.v1";phase="phase6ht";condition=$Condition;mode="proxy";status=if($qualified){"qualified"}else{"failed"};failure=$failure
