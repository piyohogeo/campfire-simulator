"""Phase 6IF exact dependency wrapper for one audit-only stage-open process."""
...
CONTRACT=SCRIPTS/"phase6if_layer_opinion_contract.json";SIDECAR=SCRIPTS/"phase6if_layer_opinion_contract.sha256"
...
if SIDECAR.read_text(encoding="ascii").split()[0].upper()!=digest:raise ImportError("phase6if_contract_digest_mismatch")
if policy.get("schema")!="campfire.phase6if.layer-opinion-contract.v1":raise ImportError("phase6if_contract_schema_mismatch")
loader_entry=policy["bootstrap_sources"]["dependency_loader"];marker_entry=policy["bootstrap_sources"]["marker_contract"]
loader=_bootstrap(ROOT/loader_entry["path"],loader_entry["sha256"],"phase6if_dependency_loader_runtime");marker_contract=_bootstrap(ROOT/marker_entry["path"],marker_entry["sha256"],"phase6if_marker_contract_runtime")
settings=carb.settings.get_settings();marker_file=Path(settings.get_as_string("/phase6if/markers")).resolve();audit_path=Path(settings.get_as_string("/phase6if/audit")).resolve();stage_root=Path(settings.get_as_string("/phase6if/stageRoot")).resolve();attempt_id=settings.get_as_string("/phase6if/attemptId")
...
modules["stage_open_source"].start_audit(authoring,modules["runtime_prim_policy"],modules["layer_opinion_audit"],modules["atomic_report"],emit,runtime_policy,audit_path,stage_root,attempt_id)
