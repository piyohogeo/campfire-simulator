"""Compare original and conditional Python state-clamp paths."""
...
DEFAULT_REPORT_JSON = (
    ...
    / "assets"
    / "phase6"
    / "phase3_state_clamp_report.json"
)
...
def _validate_group(runs: list[dict], expected_fast: bool) -> None:
    ...
    for run in runs:
        ...
        if scenario.get("wood_array_backend") != "python":
            raise ValueError("State-clamp evidence must use the Python backend")
        if scenario.get("wood_internal_timing_enabled"):
            raise ValueError("Formal comparison runs must disable internal timing")
        if scenario.get("wood_state_diagnostics_enabled"):
            raise ValueError("Formal comparison runs must disable state diagnostics")
        if not scenario.get("python_surface_boundary_fast_path"):
            raise ValueError("State-clamp evidence must use the adopted surface path")
        if bool(scenario.get("python_state_clamp_fast_path")) != expected_fast:
            raise ValueError("A comparison run used the wrong state-clamp path")
        ...
        if scenario.get("zero_area_cell_count") != {"dry": 792, "wet": 792}:
            raise ValueError("A comparison run has an unexpected zero-area count")
...
def build_report(
    ...
    for run, expected_fast in ((original_profile, False), (fast_profile, True)):
        ...
        if scenario.get("wood_state_diagnostics_enabled"):
            raise ValueError("Profile evidence enabled state diagnostics")
        if not scenario.get("python_surface_boundary_fast_path"):
            raise ValueError("Profile evidence did not use the adopted surface path")
        if bool(scenario.get("python_state_clamp_fast_path")) != expected_fast:
            raise ValueError("Profile evidence used the wrong state-clamp path")
        if not scenario.get("debugger_free"):
    ...
    return {
        ...
        "trial": {
            "original": "built-in min/max clamps",
            "fast": "conditional comparisons preserving bounds, NaN, and -0.0",
            "clamps_removed": False,
        },
        "profile": profile,
        "timings": timings,
        ...
        "decision": {
            ...
            "reason": (
                "conditional clamps improved median wood-step and scenario time "
...
def render_svg(report: dict) -> str:
    ...
    decision_text = (
        "ADOPTED · enable conditional clamp path"
        if adopted
        else "REJECTED · retain built-in min/max clamps"
    )
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="680" viewBox="0 0 1200 680" role="img" aria-labelledby="title desc">
  <title id="title">Phase 6AC conditional state-clamp decision</title>
  <desc id="desc">{report['paired_run_count']} alternating pairs compare built-in min/max clamps with equivalent conditional comparisons.</desc>
  ...
  <text x="64" y="64" class="kicker">PHASE 6AC · CONDITIONAL STATE CLAMPS</text>
  <text x="64" y="114" class="title">Keep every bound, avoid common-path calls</text>
  <text x="64" y="148" class="sub">{report['paired_run_count']} alternating pairs · debugger-free Phase 3 · NaN and −0.0 behavior preserved</text>
  ...
  <text x="92" y="374" class="small">Temperature bounds retained</text>
  <text x="92" y="402" class="small">Four mass bounds retained</text>
  <text x="92" y="430" class="small">Diagnostic counters disabled</text>
  ...
  <rect x="64" y="522" width="1072" height="94" rx="18" fill="#251a14" stroke="{decision_color}" stroke-width="2"/>
  ...
  <text x="64" y="654" class="small">Profile timing identifies the affected segment; only unprofiled end-to-end timing drives adoption.</text>
</svg>'''
