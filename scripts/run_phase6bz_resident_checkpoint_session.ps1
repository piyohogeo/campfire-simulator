...
$benchmark = Join-Path $PSScriptRoot "benchmark_resident_checkpoint_session.py"
$analyzer = Join-Path $PSScriptRoot "analyze_resident_checkpoint_session.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6bz" }
$OutputDir = [System.IO.Path]::GetFullPath($OutputDir)
$buildDir = Join-Path $OutputDir "native-build"
$raw = Join-Path $OutputDir "resident_checkpoint_session_raw.json"
...
& $kit @(
    ...
    "--/phase6bz/nativeLibrary=$nativeDll",
    "--/phase6bz/outputDir=$OutputDir",
    "--exec",
    $benchmark
)
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_checkpoint_session_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_checkpoint_session_report.svg"
