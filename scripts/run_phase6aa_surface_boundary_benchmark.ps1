...
if (-not $OriginalProfileSummary -and -not $FastProfileSummary) {
    ...
    & $phase3Runner -OutputDir $originalProfileOutput -ArrayBackend python -ProfileWoodInternals -PythonSurfaceBoundaryPath original
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
    }
    & $phase3Runner -OutputDir $fastProfileOutput -ArrayBackend python -ProfileWoodInternals -PythonSurfaceBoundaryPath fast
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
    }
...
for ($pair = 1; $pair -le $PairCount; $pair++) {
    ...
    foreach ($pathName in $paths) {
        ...
        if ($pathName -eq "fast") {
            & $phase3Runner -OutputDir $runOutput -ArrayBackend python -PythonSurfaceBoundaryPath fast
        }
        else {
            & $phase3Runner -OutputDir $runOutput -ArrayBackend python -PythonSurfaceBoundaryPath original
        }
        if ($LASTEXITCODE -ne 0) {
            exit $LASTEXITCODE
        }
