...
try {
 ...
 $monitor=Wait-CampfireKitProcessWithShutdownPolicy -Process $process -ExpectedExecutable $kit -LifecyclePath $rawOutput -LogPath $kitLog -DiagnosticDir $diagnosticDir -ShutdownGraceSeconds 60 -AbsoluteTimeoutSeconds 360
...
$evidence=[ordered]@{schema="campfire.phase6hs.kit-case-runner.v1";phase="phase6hs";mode="proxy";status=if($qualified){"qualified"}else{"failed"};failure=$failure
