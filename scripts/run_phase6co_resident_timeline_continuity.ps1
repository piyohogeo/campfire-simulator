...
$analyzer = Join-Path $PSScriptRoot "analyze_resident_timeline_continuity.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6co-resident" }
...
if ($result.status -ne "ok") { throw "Phase 6CO qualification failed: $raw" }
$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_timeline_continuity_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_timeline_continuity_report.svg"
$poster = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_timeline_continuity_frame.png"
$video = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_timeline_continuity.mp4"
& $kitPython $analyzer --raw $raw --report $report --svg $svg --poster $poster --frames $videoFrames
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$ffmpeg = Get-Command ffmpeg.exe -ErrorAction SilentlyContinue
if (-not $ffmpeg) { throw "ffmpeg.exe is required to encode the Phase 6CO video." }
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host ("Phase 6CO qualified: gates={0}, playing={1}, stopped={2}, zoom={3}-{4}" -f @($result.gates.PSObject.Properties).Count, $result.known_issue.timeline_playing_sample_count, $result.known_issue.timeline_stopped_sample_count, $result.lifecycle.timeline_zoom_configured.start_s, $result.lifecycle.timeline_zoom_configured.end_s)
