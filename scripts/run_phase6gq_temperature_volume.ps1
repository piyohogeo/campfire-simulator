...
if (Test-Path -LiteralPath $OutputRoot) { throw "Phase 6GQ refuses artifact root reuse: $OutputRoot" }
$contractPath = if ($ContractPath) { [IO.Path]::GetFullPath($ContractPath) } else { Join-Path $PSScriptRoot "phase6gq_temperature_volume_contract.json" }
...
if ($actualHash -ne $expectedHash) { throw "Phase 6GQ contract hash mismatch" }
...
& python (Join-Path $PSScriptRoot "test_phase6gq_temperature_volume.py") *> (Join-Path $preflight "fixture.log")
if ($LASTEXITCODE -ne 0) { throw "Phase 6GQ offline fixture failed before Kit launch" }
[IO.File]::WriteAllText((Join-Path $preflight "result.json"),'{"schema":"campfire.phase6gq.offline-fixture.v1","passed":true,"case_count":19}'+[Environment]::NewLine,[Text.UTF8Encoding]::new($false))
...
$attemptRoot = Join-Path $OutputRoot "formal\temperature_volume_once"
...
$state = [ordered]@{
    schema="campfire.phase6gq.incremental-state.v1";status="running";active_condition="temperature_volume_once";
    launches=1;maximum_launches=1;prior_phases_reclassified=$false;formal_population_started=$false;
...
$probe = Join-Path $PSScriptRoot "probe_phase6gq_temperature_volume.py"
...
$arguments = @(
    ...
    "-Filtering","true","-Collision","true","-Policy","allow_self_center","-ReportPhase","phase6gq",
    ...
    "-StartupProbeLabel","temperature_volume_once","-StartupFlowAcquirePosition","before_updates",
...
$guardArgs = @(
    $guard,"--trace",(Join-Path $logs "temperature_volume_once.resource.jsonl"),"--summary",(Join-Path $logs "temperature_volume_once.guard.json"),
    "--stdout",(Join-Path $logs "temperature_volume_once.stdout.log"),"--stderr",(Join-Path $logs "temperature_volume_once.stderr.log"),
    ...
    "--cpu-telemetry","--gpu-csv",(Join-Path $logs "temperature_volume_once.gpu.csv"),"--gpu-sample-ms","$($limits.gpu_sampling_ms)",
    "--lifecycle-path",(Join-Path $caseRoot "raw.json"),"--diagnostic-marker-path",(Join-Path $caseRoot "resource_markers.jsonl"),
    "--attempt-id","temperature_volume_once","--cleanup-suppression-lock",((Join-Path $caseRoot "sensitive-shutdown-diagnostics")+".ownership.json"),
...
$guardPath = Join-Path $logs "temperature_volume_once.guard.json"
...
$operationComplete = ($null -ne $operation -and $operation.status -eq "pass" -and
    $operation.operation_result -eq "pass" -and $operation.public_readback_calls -eq 1 -and
    $operation.volume_conversion_calls -eq 1 -and $operation.conversion_returned -and
    $operation.last_operation_marker -eq "phase6gq_slot0_release_after" -and
...
$operationResult = if($operationComplete -and $lifecycleNormal){"pass"}elseif($operationComplete){"partial_operation_evidence"}else{"conversion_boundary_failure"}
$lifecycleResult = if($lifecycleNormal){"normal_exit"}else{"failure"}
$summary = [ordered]@{
    schema="campfire.phase6gq.temperature-volume-summary.v1";qualified=[bool]$qualified;
    status=if($qualified){"qualified_no_later_operation_started"}else{"safe_stop"};contract_sha256=$actualHash;
    launches=1;later_operations_started=$false;formal_nine_process_population_started=$false;
    operation_result=$operationResult;lifecycle_result=$lifecycleResult;operation=$operation;
...
[IO.File]::WriteAllText((Join-Path $OutputRoot "phase6gq_summary.json"),($summary|ConvertTo-Json -Depth 16)+[Environment]::NewLine,[Text.UTF8Encoding]::new($false))
$state.status=if($qualified){"qualified_no_later_operation_started"}else{"safe_stop"}
...
[IO.File]::WriteAllText($statePath,($state|ConvertTo-Json -Depth 8)+[Environment]::NewLine,[Text.UTF8Encoding]::new($false))
if (-not $qualified) { throw "Phase 6GQ safe stop: operation=$operationResult lifecycle=$lifecycleResult" }
