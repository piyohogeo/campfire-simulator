"""Phase 6GK one-readback S93 schema and ordered alias-lifetime preflight."""
...
CONTRACT_PATH = (SCRIPT_DIR / "phase6gk_bounded_artifact_interface_contract.json").resolve()
...
try:
    discovery, discovery_audit = load_exact_module(
        DISCOVERY_PATH, DISCOVERY_PATH, module_name="campfire_phase6gk_discovery_boundary",
        required_entrypoints=("_volume_metadata", "_array_metadata", "_api_metadata"),
    )
    policy, policy_audit = load_exact_module(
        POLICY_PATH, POLICY_PATH, module_name="campfire_phase6gk_channel_policy",
        required_entrypoints=("validate_raw_schema", "validate_preflight"),
    )
    _atomic_json(audit_path, {
        "schema": "campfire.phase6gk.kit-import-audit.v1", "status": "pass",
    ...
    _atomic_json(audit_path, {
        "schema": "campfire.phase6gk.kit-import-audit.v1", "status": "fail",
...
if _sha256(CANDIDATE_PATH) != contract["candidate_schema"]["sha256"]:
    raise RuntimeError("Phase 6GK candidate schema hash mismatch")
...
def _build_preflight_stage(arguments):
    ...
    if observed != contract["condition"]["export_enable_state"]:
        raise RuntimeError(f"Phase 6GK export enable state mismatch: {observed}")
    point_summary["phase6gk_export_enable_state"] = observed
...
def _qualified_boundary(flow, volume, arguments, frame, output, collectors, operation_state=None):
    ...
    mark("phase6gk_readback_snapshot_before")
    handles = flow.get_latest_nanovdb_readback()
    mark("phase6gk_readback_snapshot_after", returned_handle_count=len(handles))
    if not isinstance(handles, list):
        raise RuntimeError(f"Phase 6GK expected public list, got {type(handles)!r}")
    if len(handles) != contract["schema_gate"]["expected_handle_count"]:
        raise RuntimeError(f"Phase 6GK expected 7 public handles, got {len(handles)}")
    rows, weak_references = [], []
    total_temporary_bytes = 0
    for index in range(len(handles)):
        mark("phase6gk_handle_snapshot_before", handle_index=index)
        ...
        mark("phase6gk_handle_snapshot_complete", handle_index=index, snapshot_path=str(snapshot_path),
        ...
        _atomic_json(snapshot_path, row)
        mark("phase6gk_handle_release_complete", handle_index=index,
    ...
    mark("phase6gk_raw_schema_validation_complete", schema_pass=raw_validation["pass"], reasons=raw_validation["reasons"])
    if not raw_validation["pass"]:
        raise RuntimeError(f"Phase 6GK raw schema validation failed: {raw_validation['reasons']}")
    ...
    mark("phase6gk_channel_preflight_durable", preflight_pass=validation["pass"],
         weak_reference_alive_count=observation["weak_reference_alive_after_release_count"],
         report_path=str(preflight_path))
    if not validation["pass"]:
        raise RuntimeError(f"Phase 6GK channel preflight failed: {validation['reasons']}")
    return {"mode": "phase6gk_s93_channel_preflight", "returned_handle_count": len(rows),
