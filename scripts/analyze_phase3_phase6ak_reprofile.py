"""Re-profile the Phase 6AK adopted path at broad and detailed timing depths."""
...
DEFAULT_JSON = ASSETS / "phase3_phase6ak_reprofile_report.json"
DEFAULT_SVG = DEFAULT_JSON.with_suffix(".svg")
PHASE6AJ_JSON = ASSETS / "phase3_adopted_reprofile_report.json"
PHASE6AK_JSON = ASSETS / "phase3_homogeneous_heat_capacity_report.json"
...
def _validate_profiles(
    ...
    for run in internal_runs + sensible_runs:
        ...
        if scenario.get("python_homogeneous_heat_capacity_fast_path") is not True:
            raise ValueError("Every profile must use the Phase 6AK homogeneous path")
    return reference
...
def build_report(
    ...
    previous = phase6aj._load(phase6aj_path)
    adopted = phase6aj._load(phase6ak_path)
    if previous.get("phase") != "phase6aj" or previous.get("status") != "ok":
        raise ValueError("Phase 6AJ profile baseline is invalid")
    if adopted.get("phase") != "phase6ak" or adopted.get("status") != "ok":
        raise ValueError("Phase 6AK adoption report is invalid")
    if adopted.get("decision", {}).get("adopt_homogeneous_path") is not True:
        raise ValueError("Phase 6AK report did not adopt the homogeneous path")
    ...
    previous_sensible = float(previous["internal_profile"]["segments"]["sensible_heat"]["median_mean_ms"])
    previous_capacity = float(previous["sensible_detail"]["segments"]["heat_capacity_evaluation"]["median_mean_ms"])

    next_audit = {
        "heat_capacity_evaluation": "loop structure and repeated public mass reads",
        "surface_boundary_update": "surface boundary arithmetic and repeated attribute reads",
    ...
    return {
        ...
        "internal_profile": {
            ...
            "phase6aj_median_total_ms": previous_internal,
            "change_since_phase6aj_percent": (
            ...
            "scenario_median_seconds": statistics.median(
                run["scenario"]["simulation_wall_seconds"] for run in internal_runs
            ),
            "segments": internal,
        ...
        "sensible_detail": {
            ...
            "sensible_change_since_phase6aj_percent": (
                (internal["sensible_heat"]["median_mean_ms"] - previous_sensible)
                / previous_sensible
                * 100.0
            ),
            "heat_capacity_change_since_phase6aj_percent": (
                (sensible["heat_capacity_evaluation"]["median_mean_ms"] - previous_capacity)
                / previous_capacity
                * 100.0
            ),
        },
        "selection": {
            ...
            "next_gate": "exact outputs plus alternating unprofiled wood-step and scenario improvement",
        },
        "equivalence": {
        ...
        "baselines": {
            "phase6aj": _relative(phase6aj_path),
            "phase6ak": _relative(phase6ak_path),
        },
        "runs": {
...
def render_svg(report: dict) -> str:
    ...
    selected = sensible[selected_name] if selected_name in sensible else internal[selected_name]
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="680" viewBox="0 0 1200 680" role="img" aria-labelledby="title desc">
  <title id="title">Phase 6AL Phase 6AK adopted-path reprofile</title>
  <desc id="desc">Three broad profiles and three detailed sensible-heat profiles rank costs after adopting step-local homogeneous heat capacity.</desc>
  ...
  <text x="58" y="56" class="kicker">PHASE 6AL - PHASE 6AK PATH REPROFILE</text>
  <text x="58" y="104" class="title">Re-rank costs after step-local specialization</text>
  <text x="58" y="137" class="sub">3 broad + 3 detailed runs - homogeneous path active - exact state, CSV, and ignition</text>
...
def main() -> int:
    ...
    parser.add_argument("--phase6aj-report", type=Path, default=PHASE6AJ_JSON)
    parser.add_argument("--phase6ak-report", type=Path, default=PHASE6AK_JSON)
    ...
    report = build_report(
        ...
        arguments.phase6aj_report,
        arguments.phase6ak_report,
    )
