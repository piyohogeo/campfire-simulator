"""Compare original and constant-model Python heat-capacity paths."""
...
DEFAULT_REPORT_JSON = (
    ...
    / "assets"
    / "phase6"
    / "phase3_heat_capacity_report.json"
)
...
def _comparison(
    ...
    fast_median = statistics.median(float(_value_at(run, path)) for run in fast)
    return {
        "original_median": original_median,
        "fast_median": fast_median,
        "improvement_percent": (
            (original_median - fast_median) / original_median * 100.0
        ),
    }


def _validate_run(run: dict, expected_fast: bool, profiled: bool) -> None:
    ...
    expected_settings = {
        ...
        "wood_internal_timing_enabled": profiled,
        "wood_sensible_heat_timing_enabled": profiled,
        "python_constant_heat_capacity_fast_path": expected_fast,
...
def build_report(
    ...
    fast_profile_path: Path,
) -> dict:
    original = _load(original_paths)
    fast = _load(fast_paths)
    if len(original) != len(fast) or len(original) < 3:
        ...
        paired.append(
            {
                ...
                "step_improvement_percent": (
                    (original_step - fast_step) / original_step * 100.0
                ),
                "scenario_improvement_percent": (
                    (original_scenario - fast_scenario) / original_scenario * 100.0
                ),
            }
        )
    improving_pairs = sum(
    ...
    for run, expected_fast in ((original_profile, False), (fast_profile, True)):
        _validate_run(run, expected_fast, True)
        for path in invariant_paths:
            if _value_at(run, path) != _value_at(reference, path):
    ...
    original_profile_ms = float(_value_at(original_profile, profile_path))
    fast_profile_ms = float(_value_at(fast_profile, profile_path))
    profile = {
        "original_mean_ms": original_profile_ms,
        "fast_mean_ms": fast_profile_ms,
        "improvement_percent": (
            (original_profile_ms - fast_profile_ms) / original_profile_ms * 100.0
        ),
    }
    ...
    return {
        ...
        "environment": {
            ...
            "adopted_runtime_paths_held_constant": True,
        },
        "trial": {
            "original": "generic heat-capacity helper",
            "fast": "constant dry-wood model specialized helper",
            "caching": False,
            "temperature_dependent_fallback": True,
            "mutable_cell_overrides_reloaded_each_call": True,
        },
        "profile": profile,
        "timings": timings,
        ...
        "decision": {
            ...
            "reason": (
                "specialized evaluation improved median wood-step and scenario time "
        ...
        "runs": {
            ...
            "original_profile": _report_path(original_profile_path),
            "fast_profile": _report_path(fast_profile_path),
        },
    }
...
def render_svg(report: dict) -> str:
    ...
    decision_text = (
        "ADOPTED - enable constant-model path"
        if adopted
        else "REJECTED - retain generic path"
    )
    return f'''<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="680" viewBox="0 0 1200 680" role="img" aria-labelledby="title desc">
  <title id="title">Phase 6AI heat-capacity path decision</title>
  <desc id="desc">Three alternating pairs compare generic and constant-model heat-capacity evaluation.</desc>
  ...
  <text x="64" y="64" class="kicker">PHASE 6AI - HEAT CAPACITY FAST PATH</text>
  <text x="64" y="114" class="title">Specialize the common model, preserve mutable state</text>
  <text x="64" y="148" class="sub">{report['paired_run_count']} alternating pairs - no cache - exact state, CSV, and ignition</text>
  <rect x="64" y="188" width="336" height="292" rx="22" fill="#272016" stroke="#a16207"/>
  <text x="92" y="230" class="heading">PROFILED HEAT CAPACITY</text>
  <text x="92" y="280" class="value">{profile['improvement_percent']:.2f}% faster</text>
  <text x="92" y="316" class="sub">{profile['original_mean_ms']:.4f} to {profile['fast_mean_ms']:.4f} ms</text>
  <text x="92" y="374" class="small">Cell override read every call</text>
  <text x="92" y="402" class="small">Temperature validated every call</text>
  <text x="92" y="430" class="small">Nonconstant models fall back</text>
  <rect x="432" y="188" width="336" height="292" rx="22" fill="#272016" stroke="#a16207"/>
  ...
  <text x="460" y="316" class="sub">{step['original_median']:.4f} to {step['fast_median']:.4f} ms</text>
  <text x="460" y="374" class="small">Median of {report['paired_run_count']} runs per path</text>
  <text x="460" y="402" class="small">No internal timers</text>
  <text x="460" y="430" class="small">Current adopted paths retained</text>
  <rect x="800" y="188" width="336" height="292" rx="22" fill="#272016" stroke="#a16207"/>
  ...
  <text x="828" y="316" class="sub">{scenario['original_median']:.4f} to {scenario['fast_median']:.4f} s</text>
  <text x="828" y="374" class="small">Flow and two captures retained</text>
  ...
  <text x="92" y="594" class="sub">No equations, grid, time step, cell order, or public mutation semantics changed.</text>
  <text x="64" y="654" class="small">Profile timing locates the effect; unprofiled end-to-end timing controls adoption.</text>
</svg>'''
