...
$analyzer = Join-Path $PSScriptRoot "analyze_resident_point_recovery.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6cj" }
...
& $kit @(
    ...
    "--/exts/campfire.app/residentPointRecoveryQualificationEnabled=true",
    "--/exts/campfire.app/residentNativeLibraryPath=$nativeDll",
    "--/rtx/flow/enabled=true"
)
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$result = Get-Content -LiteralPath $raw -Raw | ConvertFrom-Json
if ($result.status -ne "ok") { throw "Phase 6CJ failed: $raw" }
$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_recovery_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_recovery_report.svg"
$poster = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_recovery_frame.png"
$video = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_recovery.mp4"
& $kitPython $analyzer --raw $raw --report $report --svg $svg --poster $poster --frames $videoFrames
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$ffmpeg = Get-Command ffmpeg.exe -ErrorAction SilentlyContinue
if (-not $ffmpeg) { throw "ffmpeg.exe is required to encode the Phase 6CJ video." }
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host ("Phase 6CJ qualified: gates={0}, revision={1}, active blocks={2}" -f @($result.gates.PSObject.Properties).Count, $result.publication.revisions[0], $result.flow.active_blocks_peak)
