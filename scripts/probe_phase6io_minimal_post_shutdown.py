...
def _load_exact(path: Path, expected: str, name: str):
    ...
    if path.parent != SCRIPTS or _sha(path) != expected or name in sys.modules:
        raise ImportError("phase6io_exact_module_identity_invalid:" + name)
    spec = importlib.util.spec_from_file_location(name, path)
    module = importlib.util.module_from_spec(spec)
    sys.modules[name] = module
    spec.loader.exec_module(module)
    if Path(module.__file__).resolve(strict=True) != path:
        raise ImportError("phase6io_loaded_module_path_invalid:" + name)
    return module


policy = json.loads(CONTRACT.read_text(encoding="utf-8"))
if _sha(CONTRACT) != SIDECAR.read_text(encoding="ascii").split()[0].upper():
    raise ImportError("phase6io_contract_digest_mismatch")
...
boundary_spec = policy["modules"]["inherited_boundary"]
boundary = _load_exact(SCRIPTS / boundary_spec["path"], boundary_spec["sha256"], "phase6in_boundary_runtime")

settings = carb.settings.get_settings()
attempt_id = settings.get_as_string("/phase6io/attemptId")
markers = Path(settings.get_as_string("/phase6io/childMarkers")).resolve()
report_path = Path(settings.get_as_string("/phase6io/operationReport")).resolve()
expected_kit = Path(settings.get_as_string("/phase6io/expectedKitPath")).resolve(strict=True)
...
mark("kit_app_ready", {"identity_calls": 2, "stage_free": True})
report = {
    "schema": boundary.OPERATION_SCHEMA, "phase": "phase6io", "attempt_id": attempt_id,
    ...
    "operation_complete": True, "shutdown_requested": False, "shutdown_complete": False,
...
boundary.write_json(report_path, report); mark("operation_complete")
report["shutdown_requested"] = True; boundary.write_json(report_path, report); mark("shutdown_requested")
report["shutdown_complete"] = True; boundary.write_json(report_path, report); mark("shutdown_complete")
