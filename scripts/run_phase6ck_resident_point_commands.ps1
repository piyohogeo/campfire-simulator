...
$analyzer = Join-Path $PSScriptRoot "analyze_resident_point_commands.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6ck" }
...
& $kit @(
    ...
    "--/exts/campfire.app/residentPointCommandQualificationEnabled=true",
    "--/exts/campfire.app/residentNativeLibraryPath=$nativeDll",
    "--/rtx/flow/enabled=true"
)
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$result = Get-Content -LiteralPath $raw -Raw | ConvertFrom-Json
if ($result.status -ne "ok") { throw "Phase 6CK failed: $raw" }
$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_command_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_command_report.svg"
$poster = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_command_frame.png"
$video = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_point_command.mp4"
& $kitPython $analyzer --raw $raw --report $report --svg $svg --poster $poster --frames $videoFrames
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$ffmpeg = Get-Command ffmpeg.exe -ErrorAction SilentlyContinue
if (-not $ffmpeg) { throw "ffmpeg.exe is required to encode the Phase 6CK video." }
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host ("Phase 6CK qualified: gates={0}, revision={1}, active blocks={2}" -f @($result.gates.PSObject.Properties).Count, $result.publication.revisions[0], $result.flow.active_blocks_peak)
