"""Fail-closed exact repository-local loader for Phase 6HZ Kit --exec."""
...
def read_contract(wrapper_file: Path, contract_path: Path, sidecar_path: Path) -> tuple[dict, dict]:
    ...
    if _norm(contract) != _norm(scripts / "phase6hz_import_smoke_contract.json"):
        raise ImportError("import_contract_path_mismatch")
    if _norm(sidecar) != _norm(scripts / "phase6hz_import_smoke_contract.sha256"):
        raise ImportError("import_contract_sidecar_path_mismatch")
    ...
    if policy.get("schema") != "campfire.phase6hz.import-smoke-contract.v1":
...
def load_exact_module(source: Path, scripts: Path, expected_sha256: str, module_name: str, required_callables: Iterable[str]) -> tuple[ModuleType, dict]:
    ...
    if spec is None or spec.loader is None or spec.origin is None or _norm(spec.origin) != _norm(source):
        raise ImportError("probe_import_spec_or_origin_mismatch")
