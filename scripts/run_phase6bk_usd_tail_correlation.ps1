...
$analyzer = Join-Path $PSScriptRoot "analyze_usd_tail_correlation.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6bk-repro" }
...
$profiledArguments = @{
    ...
    ResidentSnapshotLightweightCommit = $true
    ResidentSnapshotSkipUnchanged = $true
    ResidentSnapshotLightweightTailTiming = $true
    ResidentNativeBackend = $true
    ResidentNativeLibraryPath = $nativeDll
}
for ($index = 1; $index -le $RunCount; $index++) {
    $controlDir = Join-Path $OutputDir ("control-{0:D2}" -f $index)
    $profiledDir = Join-Path $OutputDir ("profiled-{0:D2}" -f $index)
    if ($index % 2 -eq 1) {
        & $phase3Runner -OutputDir $controlDir @controlArguments
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
        & $phase3Runner -OutputDir $profiledDir @profiledArguments
        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    }
    else {
        & $phase3Runner -OutputDir $profiledDir @profiledArguments
