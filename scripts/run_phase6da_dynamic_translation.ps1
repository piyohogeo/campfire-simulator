...
$analyzer = Join-Path $PSScriptRoot "analyze_phase6da_dynamic_translation.py"
...
if (-not $OutputDir) { $OutputDir = Join-Path $repositoryRoot "artifacts\phase3\phase6da-dynamic-translation" }
...
$log = Join-Path $OutputDir "phase6da.log"

if (-not (Test-Path -LiteralPath $kit) -or -not (Test-Path -LiteralPath $app)) { throw "Application is not built." }
if (-not (Test-Path -LiteralPath $nativeLibrary)) { throw "Phase 6DA requires the existing Phase 6CO native build: $nativeLibrary" }
...
if ($productionHashBefore -ne $productionHashAfter) { throw "Phase 6DA changed the production app file." }

$probeResult = Get-Content -LiteralPath $raw -Raw | ConvertFrom-Json
if ($probeResult.status -ne "ok") { throw "Phase 6DA probe failed: $raw" }
if (-not (Test-Path -LiteralPath $base)) { throw "Phase 6DA base summary was not written." }

$report = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_dynamic_translation_report.json"
$svg = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_dynamic_translation_report.svg"
$poster = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_dynamic_translation_frame.png"
$video = Join-Path $repositoryRoot "docs\devlog\assets\phase6\resident_dynamic_translation.mp4"
& $kitPython $analyzer --raw $raw --base $base --report $report --svg $svg --poster $poster --frames $frames --production-sha256-before $productionHashBefore --production-sha256-after $productionHashAfter --kit-exit-code $kitExitCode
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
$ffmpeg = Get-Command ffmpeg.exe -ErrorAction SilentlyContinue
if (-not $ffmpeg) { throw "ffmpeg.exe is required to encode the Phase 6DA video." }
...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host ("Phase 6DA dynamic translation: alignmentMm={0}, layout={1}->{2}" -f ($probeResult.boundary.post.maximum_alignment_error_m * 1000.0), $probeResult.boundary.pre.layout_revision, $probeResult.boundary.post.layout_revision)
