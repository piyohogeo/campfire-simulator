...
次の作業は、実環境の実験検証を保留したまま、外部面積が0の内部セルを放射・対流の境界計算から早期に外す等価試作を行い、交互順序の非計測Phase 3でend-to-end改善を確認することです。NumPyは明示選択可能な検証経路として残しますが、現AoS境界では既定化しません。Warpは全状態をGPU常駐させ、CPU側のメトリクス・Flow・USD境界まで再設計できる場合に限って再検討します。責任研究室からレビュー済み票を受領するまで設備dry runは開始しません。
