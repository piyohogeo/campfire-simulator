...
現在は **Phase 6「校正と品質」進行中** です。公称12.7 mm・5層の合板試験片で、公開されたガス・タール・チャーの3つの一次Arrhenius反応を同時に解きます。各層の乾燥木材消費率から未収縮の質量等価熱分解深さを診断し、34.7 kW/m²・600秒のカバ合板炭化深さ実測と非採点で比較します。試験片・環境・深さ定義が一致しないため物理厚さとDarcy滞留時間は保留しています。
...
次の作業はNIST試験片に近い外装5層合板について、樹種・接着剤・初期厚さ・密度・含水率を伴う厚さ方向収縮または断面炭化深さの直接測定を探すことです。一致する資料がなければ、専用試験の計測仕様を定義します。空隙率・透過率、高温混合気粘度、圧力差も直接根拠を確保してから追加します。
