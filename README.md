...
- Phase 6U時点で全40テストはcoverageなし`31.281 s`で成功します。標準`repo.bat test`のcoverage付き経路は、NIST校正テストがランナー内蔵`300 s`上限へ達する既知の検証性能問題があります。同じ単体テストはcoverageなし`23.719 s`で成功しており、数値不一致ではありません。
...
次の作業は、実環境の実験検証を保留したまま、Phase 6Uで有利だったNumPy AoS往復を実際の`WoodThermalModel.step()`へ限定試作し、伝導・蒸発・反応を含む全stepとPhase 3で再測定することです。状態SHA-256が一致しない場合は採用しません。Warpは全状態をGPU常駐させ、CPU側のメトリクス・Flow・USD境界まで再設計できる場合に限って再検討します。責任研究室からレビュー済み票を受領するまで設備dry runは開始しません。
