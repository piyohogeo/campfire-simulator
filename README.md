...
現在は **Phase 6「校正と品質」進行中** です。NISTIR 7094の合板反復試験SAMP.1/2だけで係数候補を選び、未使用のSAMP.3と別材料OSBへ再調整なしで適用する2段階のホールドアウトを再現可能に比較できます。
...
Phase 6検証は[NISTIR 7094](https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=101408) Appendix Aの35・70 kW/m²合板反復試験を固定分割し、SAMP.1/2で36候補を評価、未使用のSAMP.3で同一材料内の再現性を測ります。選択後の係数はOSBにも再調整なしで適用し、データ隔離、着火順、質量保存、有限値と、USD・PNG・3種のSVG・CSV・JSON生成を検査します。
...
次の作業は等価円柱を、試験片の実厚さと5層構造を明示するモデルへ置き換え、温度依存の反応速度を導入することです。その後、追加条件とFDSで再検証し、描画品質、性能最適化を扱います。
