...
現在は **Phase 6「校正と品質」進行中** です。公称12.7 mm・5層の合板試験片へ、一次のArrhenius熱分解速度`k(T)=A exp(-E/RT)`を導入しました。公開された3組の木材分解経路から48候補を作り、SAMP.1/2だけで選定、未使用SAMP.3と別材料OSBへ再調整なしで適用します。
...
固定シーンとFlow・剛体設定に加え、熱伝導のエネルギー保存、負質量・NaN防止、湿潤薪の着火遅延、5層試験片の層間熱伝導、Arrhenius速度の温度単調性、USD状態の保存再読込をテストします。
...
Phase 6検証は[NISTIR 7094](https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=101408) Appendix Aの35・70 kW/m²合板反復試験を固定分割します。公称12.7 mm・5層モデルで、3組の公開A/E値×4段階のA倍率×4吸収率=`48候補`をSAMP.1/2だけで評価し、未使用SAMP.3で再現性を測ります。選択後の係数はOSBにも再調整なしで適用し、データ隔離、Arrhenius単位、着火順、質量保存、有限値と、USD・PNG・5種のSVG・CSV・JSON生成を検査します。
...
- Phase 6Dの12.7 mmは、報告書に記載された元の屋根パネル公称厚をコーン試料にも適用した推定です。5層は等厚、木目は交互方向と仮定し、接着層と温度依存Arrhenius反応はまだ明示していません。
- Phase 6EはThurner–Mannのガス・タール・炭3経路のA/E組を、個別の見かけの一次反応候補として使う簡略モデルです。元論文の3経路を同時に解くモデルではなく、選定Aが探索上限の16倍に達したため係数同定も完了していません。
...
次の作業はガス・タール・炭の3経路を同時に解き、反応分率と生成物収率を一次資料から固定することです。接着層とOSB固有物性も分離し、その後、追加条件とFDSで再検証します。
