...
現在は **Phase 6「校正と品質」進行中** です。公称12.7 mm・5層の合板試験片で、公開されたガス・タール・チャーの3つの一次Arrhenius反応を同時に解きます。35/70 kW/m² × 60/180/300/600秒 × 3反復の24条件について、初期面基準の座標、DAQ基準時刻、質量・温度・表面変位・断面画像の生データ契約とRun IDを固定しました。実測は0/24件で、責任研究室による実験検証は保留しています。現在は物理条件を変えず、CPU木材モデルとFlow入力変換の性能改善を進めています。
...
次の作業は、実環境の実験検証を保留したまま、Flow／USD／キャプチャ経路の更新コストをさらに分離することです。CPUホットパスの測定結果を基準に、配列化またはWarp化が実際に必要な箇所だけを選びます。責任研究室からレビュー済み票を受領するまで設備dry runは開始しません。
