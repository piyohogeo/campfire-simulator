...
現在は **Phase 6「校正と品質」進行中** です。NISTIR 7094の公称12.7 mm屋根合板を、0.1 m角・5層の平板試験片としてモデル化しました。SAMP.1/2だけで係数を選び、未使用のSAMP.3と別材料OSBへ再調整なしで適用する2段階のホールドアウトを再現可能に比較できます。
...
固定シーンとFlow・剛体設定に加え、熱伝導のエネルギー保存、負質量・NaN防止、湿潤薪の着火遅延、5層試験片の層間熱伝導、USD状態の保存再読込をテストします。
...
Phase 6検証は[NISTIR 7094](https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=101408) Appendix Aの35・70 kW/m²合板反復試験を固定分割し、公称12.7 mm・5層の平板モデルでSAMP.1/2の36候補を評価、未使用のSAMP.3で同一材料内の再現性を測ります。選択後の係数は1層平板のOSBにも再調整なしで適用し、データ隔離、層数・厚さ、着火順、質量保存、有限値と、USD・PNG・4種のSVG・CSV・JSON生成を検査します。
...
次の作業は区分線形の熱分解速度を、出典と単位を持つ温度依存Arrhenius反応へ置き換えることです。接着層とOSB固有物性を分離し、その後、追加条件とFDSで再検証します。
