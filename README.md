...
現在は **Phase 6「校正と品質」進行中** です。NISTIR 7094の5層合板データだけで係数候補を選び、候補選択に使わないOSBへ再調整なしで適用する外部材料ホールドアウトまで再現可能に比較できます。
...
Phase 6検証は[NISTIR 7094](https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=101408) Table 2の35・70 kW/m²合板データで固定36候補を評価します。選択後の係数をOSBへ再調整なしで適用し、データ隔離、着火順、質量保存、有限値と、USD・PNG・校正/ホールドアウトSVG・CSV・JSONの生成を検査します。
...
次の作業はNISTIR 7094 Appendix Aの合板反復試験を固定分割し、同一材料内のホールドアウトを作ることです。その後、試験片形状と反応速度モデル、FDS比較、描画品質、性能最適化を扱います。
