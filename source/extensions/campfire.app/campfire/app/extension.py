...
class CampfireAppExtension(omni.ext.IExt):
    ...
    def on_shutdown(self):
        ...
        if self._resident_snapshot_adapter is not None:
            ...
            self._resident_snapshot_adapter = None
        if self._wood_visual_v0_consumer is not None:
            try:
                self._wood_visual_v0_consumer.close()
            except Exception as exc:
                carb.log_error(
    ...
    async def _initialize(self):
        ...
        try:
            ...
            if point_application_enabled and settings.get_as_bool(
                WOOD_VISUAL_V0_SETTING
            ):
                raise ValueError(
    ...
    async def _initialize_resident_point_stage(
        ...
        *,
        ...
        try:
            ...
            skip_unchanged_translation_layout = point_settings.get_as_bool(
    ...
    async def _run_resident_point_application(
        ...
        def register_point_listener(target_stage):
            return Tf.Notice.Register(
                Usd.Notice.ObjectsChanged, observe_point_changes, target_stage
            )
        ...
        try:
            ...
            if (
                ...
                layout_changed_point_count = sum(
                    before != after
            ...
            gates = {
                ...
                "normal_extension_owner_composed": (
                    stopped_status["start_count"] == expected_owner_transitions
                    and stopped_status["stop_count"] == expected_owner_transitions
            ...
            if recovery_qualification:
                ...
                gates.update(
                    {
                        "stopped_layout_refresh_committed": (
                        ...
                        "normal_owner_stage_recovery_completed": (
            ...
            if not all(gates.values()):
                ...
                raise RuntimeError(f"{phase_name.upper()} gates failed: {failed}")
            carb.log_info(
                f"[campfire.app] {phase_name.upper()} complete: "
    ...
    async def _run_phase3(self, viewport, stage, scene_path: Path):
        ...
        if wood_visual_v0_enabled and not resident_snapshot_adapter_enabled:
            raise ValueError(
        ...
        if resident_snapshot_adapter_enabled:
            ...
            if wood_visual_v0_enabled:
                wood_visual_consumer = WoodVisualV0Consumer(
        ...
        try:
            ...
            for step_index in range(1, PHASE3_TOTAL_STEPS + 1):
                ...
                if update_flow:
                    ...
                    if resident_adapter is not None:
                        ...
                        if wood_visual_consumer is not None:
                            try:
                                wood_visual_profiles.append(
                                    wood_visual_consumer.publish(snapshot)
                                )
                            except Exception as visual_error:
                                wood_visual_errors.append(
                                    {
                                ...
                                carb.log_error(
                                    "[campfire.app] Wood visual V0 publication failed "
                                    f"at revision {snapshot.revision}: {visual_error}"
                                )
            ...
            summary = {
                ...
                "scenario": {
                    ...
                    "wood_visual_v0": {
                        "enabled": wood_visual_v0_enabled,
                        "input": "ResidentPublishedSnapshot",
                        "status_after_timeline_stop": wood_visual_status,
                        "publication_timing": (
                            summarize_timing_ms(
                                [profile.total_ms for profile in wood_visual_profiles],
                                min(1, len(wood_visual_profiles)),
                            )
                            if wood_visual_profiles
                            else None
                        ),
                        "usd_set_count": sum(
                            profile.usd_set_count for profile in wood_visual_profiles
                        ),
                        "skipped_set_count": sum(
                            profile.skipped_set_count
                            for profile in wood_visual_profiles
                        ),
                        "notice_count": sum(
                            profile.notice_count for profile in wood_visual_profiles
                        ),
