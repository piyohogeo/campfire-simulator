...
def evaluate_parameters(
    reference: dict, parameters: WoodModelParameters
) -> dict:
    cases = []
    squared_errors = []
...
def run_nist_plywood_calibration() -> dict:
    ...
    evaluated = [
        evaluate_parameters(reference, candidate)
        for candidate in calibration_candidates()
    ]
    evaluated.sort(key=lambda item: item["score_rmse_relative"])
    best = evaluated[0]
    return {
        "reference": {
            key: reference[key]
...
def write_calibration_svg(calibration: dict, destination: Path) -> Path:
    ...
    lines = [
        ...
        '<text x="60" y="58" font-size="30" font-weight="700">Phase 6A — NIST plywood calibration baseline</text>',
        '<text x="60" y="88" class="muted" font-size="16">NISTIR 7094 Table 2 · equivalent cylindrical coupon adapter</text>',
    ]
    for label, color in colors.items():
        ...
        lines.extend(
            (
                ...
                f'<text x="{x + 27}" y="77" font-size="14">{label.title()}</text>',
            )
        )
    ...
    baseline_score = baseline["score_rmse_relative"]
    best_score = best["score_rmse_relative"]
    improvement = calibration["improvement_fraction"] * 100.0
    lines.extend(
        (
            f'<text x="60" y="625" font-size="17">Relative RMSE: baseline {baseline_score:.3f} → calibrated {best_score:.3f} ({improvement:.1f}% improvement)</text>',
            '<text x="60" y="652" class="muted" font-size="14">Scope: proxy ignition and equivalent geometry; this is a reproducible calibration baseline, not full plywood validation.</text>',
            '</svg>',
        )
    )
