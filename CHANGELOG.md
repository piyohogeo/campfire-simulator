...
- Safely stopped the fresh channel preflight when public NanoVDB readback returned seven handles but the frozen channel schema expected six. No handle indexing, conversion, NPZ, spatial metric, formal run, replacement, or video followed. Stage close completed in 15.656 seconds; CDB/fatal/dump/upload/residual were zero and exact cleanup succeeded.
- Preserved historical shared-probe SHA contracts by moving Phase 6GC behavior into an isolated shared probe. Release build, Phase 0 RTX, Phase 3 (zero mass-balance error), focused Phase 6F 212/212, focused Phase 6G 16/16, the standard 8-process 78/78 suite, and devlog validation passed. Production/latest-demo hashes were unchanged and final Kit/CDB/GPU-helper residuals were zero.
...
- Safely stopped Phase 6GB before readback and before all nine formal slots when the frozen absolute startup source-sum gate rejected float32 fuel `1075.2000160217285` versus `1075.2` at tolerance `1e-5`. The geometry mapping was correct and Flow was representative at 688 active blocks; the threshold was not changed and no retry or replacement ran.
- Preserved the complete release-after-close and exact-cleanup evidence: Kit/tree peaks were 13,461,442,560/13,613,182,976 bytes, stage close and `shutdown_complete` completed, OS exit was deliberately nonzero, and CDB/fatal/dump/upload/residual counts were zero. S93/S100/OFF numeric and visual conclusions remain unavailable and production is unchanged.
- Reconfirmed the boundary with Release build, Phase 0 RTX, Phase 3 zero dry/wet mass balance and wood-owned Flow input, focused Phase 6F 212/212 plus Phase 6GB 6/6, the standard eight-process 78/78 suite, and devlog validation. Production/defaults/latest demo hashes remained unchanged and final Kit/CDB/GPU-helper residual count was zero.
...
- Reconfirmed the safe stop with Release build, Phase 0 RTX, Phase 3 zero mass-balance and wood-owned Flow input, 212/212 focused Phase 6F contracts, the standard 8-process 78/78 suite, and devlog validation. Final Kit/CDB/nvidia-smi residual count was zero.

- Fixed the Phase 6FY Kit `--exec` harness import boundary with deterministic file-based loading, exact resolved-path/entry-point audits, and a real app-ready positive/missing/wrong-path smoke gate. Added progress-aware bounded CDB passes with separate no-progress and absolute timeouts, retained partial evidence, independent module classification, explicit detach, and exact child cleanup.
- Qualified the fresh Phase 6FZ balanced M0/M1/M2 population: 9/9 memory-valid normal exits, zero replacement/timeout/CDB/fatal/dump/upload/residual, Kit peak 14,478,200,832--14,957,187,072 bytes, and tree peak 15,120,756,736 bytes. The old 14 GiB threshold is retired as too close to normal high-water; the predeclared 16 GiB Kit and 17 GiB tree limits are qualified with 2,222,682,112 and 3,132,854,272 bytes of headroom.
- Reconfirmed the result with the Release build, Phase 0 RTX, Phase 3 zero dry/wet mass balance and wood-owned Flow input, 212/212 focused Phase 6F tests, the standard eight-process 78/78 suite, and static devlog validation. Production/defaults/latest demo remained unchanged and final Kit/CDB/GPU-helper residual count was zero; Phase 6FO was not started.

- Froze the Phase 6FW post-cleanup PID-reuse policy without reclassifying Phase 6FV or changing the Phase 6FU seven-state query and exact-cleanup implementation. A protected non-residual now requires a complete trusted current identity, a clear creation-time or safely normalized absolute-path mismatch, no matching/conflicting/unknown evidence, no stop request for the new identity, ordered cleanup markers, released suppression, and zero attempt-owned residual.
- Added a pre-runtime SHA-pinned contract, conservative Windows path and UTC creation-time comparison, fifteen fail-closed fixture definitions, and a bounded separate-process fixture runner. Phase 6FV attempt03 will be compared offline by source hash only; Kit, CDB, Phase 6FO, the memory population, production, defaults, and video remain outside this phase.
- Qualified Phase 6FW with 15/15 isolated fixtures and the read-only Phase 6FV-equivalent comparison. The latter produced 39 confirmed-absent identities, one protected PID reuse, zero matching residual, zero unresolved unknown, zero mismatched-identity stop request, released suppression, and zero final attempt-owned residual; Phase 6FV itself remains the frozen safe stop.
- Kept the Phase 6FU/legacy guard and Phase 6FM/6FN/6FV contract hashes unchanged. Release build, Phase 0 RTX, Phase 3 zero mass-balance and wood-owned Flow input, 185/185 focused Phase 6F tests, the eight-process 78/78 standard suite, and static devlog validation passed. Production/defaults/latest demo were unchanged, residual processes were zero, and the nine-process memory population was not started.

- Added the fresh Phase 6FV post-6FU nine-process memory-ceiling contract without reclassifying Phase 6FT. The frozen population reused `release-after-close`, the dedicated Phase 6FU exact-identity guard, stack-first CDB, corrected S93 four-log fixture, balanced M0/M1/M2 order, and readback/capture/video zero. Kit 16 GiB and tree 17 GiB remained absolute stops; Phase 6FO and production were not started or changed.
- Safely stopped Phase 6FV at attempt03 M2 when exact cleanup detected that an observed `conhost.exe` PID had been reused by a later `WmiPrvSE.exe`. The guard did not terminate the mismatched process; matching residual, final unknown, and killed PID counts were zero, dual-source absence completed, and all three launched Kit processes had already exited normally. The predeclared no-mismatch integrity gate therefore stopped attempts04–09 without retry.
- Preserved 16/17 GiB as unqualified because the required 9/9 population did not complete. The two representative runs peaked at 14,899,392,512–14,945,353,728 Kit bytes, leaving only 87,031,808 bytes below 14 GiB and confirming that the old anomaly threshold remains too close to normal high-water. Phase 6FO remains blocked; no readback, CDB, fatal, dump, upload, video, latest-demo, default, or production change occurred.
- Qualified the Phase 6FU hang-diagnostic identity and cleanup boundary without starting Kit, Phase 6FO, or the nine-process memory population. Stack-first CDB retained complete native stacks across an auxiliary module timeout, explicit detach preserved the target, seven identity/cleanup fixture groups passed, and dual psutil/native-Win32 absence confirmation completed with zero residual.
- Replaced binary process-presence handling with seven fail-closed states, exact PID/creation-time/path/parentage/attempt ownership, bounded diagnostic cleanup suppression, partial diagnostic artifacts, and summary-after-cleanup ordering. Runner and diagnostic peaks were 9,482,240 and 94,515,200 bytes; full dump and automatic upload remained disabled.
- Restored the shared Phase 6EG resource guard to its frozen SHA-256 and isolated the new behavior in a Phase 6FU adapter, preserving Phase 6FM/6FN runtime hashes. Phase 6FT remains a frozen 8/9 safe stop, 16/17 GiB remains unqualified, Phase 6FO remains blocked, and production/defaults/video/latest demo remain unchanged.
- Added Phase 6FT as a frozen, production-neutral memory-ceiling qualification using the Phase 6FS `release-after-close` order. Eight balanced M0/M1/M2 readback-free processes exited normally; their Kit peaks were 14,494,695,424–14,986,518,528 bytes, and no condition-following Phase 6FO preallocation was observed.
- Safely stopped Phase 6FT at attempt09 when the third M1 control timed out at stage close after 180.021 seconds. No resource ceiling was approached, but bounded CDB evidence did not finalize and the guard's cleanup report conflicted with later exact process enumeration; the verified attempt tree was then removed and residual count was confirmed zero.
- Classified 14 GiB as too close to normal four-log high-water (43.742 MiB minimum normal margin), while declining to qualify the 16 GiB Kit / 17 GiB tree candidates because the frozen 9/9 lifecycle and cleanup gate failed. Phase 6FO, S93/S100 comparison, video, P4, production defaults, and production shutdown order remain unchanged.
- Reconfirmed the safe stop with Release build, Phase 0 RTX, Phase 3 zero mass-balance error and wood-owned Flow input, 161/161 focused Phase 6F contracts, the standard 78/78 suite, and devlog validation. Production SHA-256 remained unchanged and final Kit/CDB/GPU-helper residual count was zero.

- Added and completed the Phase 6FS B-first `release-after-close` limited qualification without reclassifying Phase 6FR or restarting Phase 6FO. Three independent readback/capture-free S93 processes retained explicit stage/viewport/Flow/provider/Emitter/collector ownership through stage close, USD detach, and four post-close updates, then cleared every Python-owned slot and exited normally. Stage-close times were 2.224, 2.917, and 21.879 seconds; CDB, fatal, dump, upload, cleanup, and residual counts were zero.
- Kept the 14 GiB Kit and 16 GiB tree ceilings unchanged. The maximum Kit peak was 14,941,323,264 bytes, leaving 86.844 MiB to 14 GiB, and the maximum tree peak was 15,105,253,376 bytes. `release-after-close` is a strong candidate for the next limited memory-ceiling qualification, not a production change or proof that the low-frequency native lifecycle issue is solved. S93/S100 comparison, video, latest demo, P4, and Phase 6FO remain untouched.
- Reconfirmed Phase 6FS with Release build, Phase 0 RTX, Phase 3 zero dry/wet mass-balance error, 154/154 focused Phase 6F contracts, the standard 78/78 suite, and devlog asset validation. The production app SHA-256 remained unchanged and final Kit/CDB/GPU-helper residual count was zero.

- Froze the Phase 6FR stack-first CDB and minimal shutdown-order contract without reclassifying Phase 6FQ or restarting Phase 6FO. The bounded diagnostic now captures all-thread native stacks before auxiliary module enumeration, uses a local-only symbol cache, preserves stack evidence across a module timeout, and verifies an independent explicit detach. A stoppable fixture is a mandatory gate before the six-process `AB / BA / AB` release-before/after-close comparison.
- Passed all Phase 6FR CDB fixtures, then safely stopped the formal population at the first release-before-close A control. `close_stage_async()` timed out after 180.014 seconds with no readback, capture, or resource-ceiling failure. The complete 149-thread stack narrowed the boundary to `omni_usd!UsdManager::destroyContext` and an exclusive-SRW wait in the `UsdContext` loader thread; the owner remains unknown and the accepted NGX signature did not match. Exact detach and cleanup left zero residual. The frozen fail-fast order prevented B from starting, so release-after-close and memory-ceiling qualification remain blocked.
- Reconfirmed the Phase 6FR safe stop with Release build, Phase 0 RTX, Phase 3 zero dry/wet mass-balance error, 68/68 focused safety contracts, the standard eight-process 78/78 suite, and static devlog checks. Production SHA-256 and defaults remained unchanged; no video/latest-demo update or residual process occurred.

- Added the default-off Phase 6FQ contract and isolated harness to separate the Phase 6FP C5 stage-close timeout from memory-ceiling calibration. Six readback-free controls exited normally, including disabled capture metadata, manifest, provider-alias, and no-drain variants, before the ordinary 8-update/release-before-close control reproduced a 180.024-second stage-close timeout without capture preparation. Bounded CDB module capture timed out before a usable stack, exact cleanup succeeded, memory-ceiling qualification did not start, and Phase 6FO remains blocked.
...
- Defined the default-off Phase 6FO roadmap-P3 comparison without reclassifying Phase 6EP–6ES or extending Phase 6FN beyond three readbacks. The frozen corrected-four fixture compares S93 support-clear (1,344/1,440 Points) with S100 center-clear (1,440/1,440 Points), preserves all production paths and ceilings, and records every Point decision before runtime.
- Added a bounded four-channel preflight and formal spatial path for direct public velocity/temperature/smoke/fuel buffers. It performs no `np.asarray` or requested material copy, releases each channel alias after its compact near-Mesh evidence is durable, disables forced GC for the new path, and fails closed on pointer, weak-reference, resource, lifecycle, or cleanup evidence.
- Froze the first Phase 6FO root as a harness safe stop before readback and before all formal processes. Corrected the float32 startup-sum tolerance semantic and the `channel_attempt*` analyzer route in contract v2; physical comparison thresholds and all safety ceilings remain unchanged.
- Safely stopped the fresh Phase 6FO v2 root at the unchanged 14 GiB Kit hard gate before the first channel readback. Startup was representative (688 blocks at frame 60; 948 at frame 96), but the guard observed 15,127,232,512 bytes, 90.453 MiB over the ceiling. No formal S93/S100 process or video ran; exact cleanup completed with no fatal, dump, upload, or residual, and production remained unchanged.
- Reconfirmed the safe stop with the Release build, Phase 0 RTX, Phase 3 (zero dry/wet mass-balance error), 359/359 focused Phase 6 contracts, the standard 8-process 78/78 suite, and static devlog validation. Production SHA-256 remained unchanged and final residual count was zero.

- Added the pre-runtime Phase 6FN routed settled-baseline contract without reclassifying Phase 6FL/6FM or reusing Phase 6FM attempt01. Formal decisions are split into schema routing, startup, operation, explicit settling, pointer/alias, paired accumulation, absolute safety, lifecycle, and cleanup layers; the legacy Phase 6FL evaluator is diagnostic-only.
- Fixed the formal iteration-3 boundary to consume `settling_end(frame=620, settling_iteration=3)` while requiring frame 620 to remain free of control/readback/NumPy operation markers. The 4-second, 8-resource-sample, 60-update settled baseline and unchanged 91,184,640-byte paired material threshold are frozen in contract SHA-256 `33D7A175...BC0247`.
- Passed a 19-check end-to-end preflight using the actual raw JSON, JSONL marker, process summary, guard, and resource-trace directory shape. It covers the complete nine-process pass shape and fail-closed mutations for settling, operation, pointer, weak reference, lifecycle, cleanup, ceilings, parse/routing, and launch-after-stop behavior.
- Production, Flow, Point payload, CollisionProxy, V3, resource ceilings, and the latest demo remain unchanged. Formal Kit runtime requires the matching preflight manifest and a new empty artifact root.
- Completed the fresh Phase 6FN balanced population in nine launches with no startup replacements. All 27 operation/explicit-settling intervals, all nine R2 pointer-complete aliases, all paired field-adjusted sequence comparisons, and all normal-exit/cleanup gates passed; no CDB, fatal, dump, upload, or residual occurred.
- Qualified exactly three public readbacks and three ordered-release fuel zero-copy aliases per process for the fixed fixture. `np.asarray()` adjacent CPU delta was 0–1,052,672 bytes and GPU delta was 0 MiB; field growth from 41,398,016 to 55,666,688 bytes was retained and adjusted in the paired comparison. Four or more iterations, other channels, long retention, field persistence, and production integration remain unqualified.
- Observed a 14,918,832,128-byte Kit peak, leaving 113,553,408 bytes below 14 GiB. Stage close was 2.501–34.329 seconds (median 4.097), all below 180 seconds. Production SHA-256 remained unchanged.

- Added the frozen Phase 6FM settled post-release contract without changing Phase 6FK/6FL history. It records independent `pre_operation`, operation, release, settling-start, and explicit settling-end markers, keeps frame 620 readback-free, and limits formal accumulation failure to replicated R0-subtracted, field-adjusted settled staircases.
- Safely stopped the new Phase 6FM root after its first R0 launch. All three explicit settling intervals passed (51/39/18 resource samples and 240/180/80 updates), stage close took 3.339 seconds, and the process exited normally with no fatal, dump, upload, CDB, or residual.
- Identified the fail-closed boundary as an analyzer integration mismatch: the reused Phase 6FL compatibility pass still required legacy `sample_started` at the frame-620 non-operation sentinel, although the new explicit `settling_end` marker was present and valid. The attempt remains an operation/harness safe stop under the frozen contract; later eight slots were not launched and no retry or reclassification was performed.
- Kept three-readback, three-fuel-alias, longer repetition, and production integration unqualified. Production, Flow, Point payload, CollisionProxy, V3, resource ceilings, and the latest demo remain unchanged.
- Reconfirmed the Phase 6FM safe stop with the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow checks, 167/167 focused Phase 6F/6EA/6EB/6EL contracts, the standard eight-process 78/78 suite in 348.9 seconds, and static devlog validation. Production SHA-256 remained unchanged and final Kit/CDB/GPU-helper residual count was zero.

- Added the frozen Phase 6FL three-iteration pilot contract without reusing or reclassifying Phase 6FK. It fixes exactly three operations per process, balanced `R0/R1/R2`, `R1/R2/R0`, `R2/R0/R1` ordering, paired R0-subtracted accumulation gates, bounded settling, pointer/weak-reference evidence, and the unchanged 14/16 GiB safety ceilings.
- Preserved three distinct pre-formal safe stops: an empty PowerShell argument, a shared-runner compatibility label rejection, and an analyzer mismatch between extension marker keys. None launched or reclassified formal evidence; the corrected contract SHA-256 is `8F2BB5FD237CB162865B7FCBAB3B75AC6C54C9A86C4753BFAC7FA85D6BEFDB62`.
- Safely stopped the fresh Phase 6FL formal population after sequence 1. R0/R1/R2 each completed three exact operations, normal stage close, extension shutdown, OS exit, and cleanup, but R1 and R2 exceeded the frozen paired pre-operation two-step accumulation gate. Later six processes were not launched; three-readback and three-alias lifetimes remain unqualified.
- Retained the partial pointer evidence: all three R2 iterations used positive equal source/converted pointers, the same Python object and shared memory, zero weak-reference residual, zero GPU delta, and no positive adjacent `numpy.asarray()` CPU allocation. Production and the latest demo remain unchanged.
- Reconfirmed the safe stop with the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow checks, 156/156 focused Phase 6F/6EA/6EB/6EL contracts, the eight-process 78/78 standard suite, and static devlog validation. Production SHA-256 remained unchanged and final Kit/CDB/GPU-helper residual count was zero.
...
- Completed the fresh Phase 6FK balanced population with nine representative normal exits and no replacements. All three C cases persisted positive equal source/converted pointers, same-object shared-memory aliases, zero adjacent `numpy.asarray()` CPU/GPU increase, and zero weak-reference residual; one public readback and one fuel-alias lifetime are qualified for the fixed fixture.
- Preserved repeated readback as unqualified. The worst Kit margin was 132,071,424 bytes and one B stage close took 140.481 seconds under the frozen 180-second bound, so the resource and low-frequency native lifecycle risks remain explicit. Production and the latest demo are unchanged.
- Final regression passed the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow checks, 143/143 focused safety contracts, the eight-process 78/78 standard suite, and static devlog validation. Production SHA-256 remained unchanged and final Kit/CDB/GPU-helper residual count was zero.
...
- Safely stopped Phase 6FJ after the post-runtime integrity audit found that all three C artifacts lacked the mandatory source/converted NumPy data-buffer pointers. Same Python identity, shared memory, zero adjacent `np.asarray()` increase, and zero weak-reference residual remain partial evidence only; neither single readback nor fuel-alias lifetime is qualified.
- Added bounded public NumPy data-pointer evidence to the diagnostic probe and made missing or inconsistent pointer evidence a nonreplaceable fail-closed operation failure. Production, the frozen contract, prior artifacts, and latest demo remain unchanged.
- Reconfirmed the safe stop with the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, 137/137 focused Phase 6F/6EA/6EB/6EL contracts, and the standard eight-process 78/78 suite. Production SHA-256 stayed unchanged, devlog validation passed, and no Kit/CDB/GPU-helper residual remained.
...
- Representative stage closes ranged from 2.027 to 8.551 seconds. Native lifecycle, operation, absolute-safety, CDB, fatal, access-violation, dump, upload, and cleanup-residual counts were zero. This is bounded non-reproduction in six representative runs, not proof that the Phase 6FG issue is absent; Phase 6FG remains explicit-approval gated.
- Reconfirmed Phase 6FI with the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, 126/126 focused Phase 6F/6EA/6EB/6EL contracts, and the standard eight-process suite 78/78 in 315.3 seconds. Static devlog validation passed, production SHA-256 and contract SHA-256 remained unchanged, and no Kit/CDB/GPU helper remained.
...
- Recorded that the Phase 6FG native stage-close failure did not reproduce in this invalid startup sample, but no lifecycle incidence can be inferred from zero representative controls. Phase 6FG remains frozen and its balanced population requires explicit approval under a new root.
- Reconfirmed Release build, focused Phase 6F/6EA/6EB/6EL 117/117, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, and the standard suite 78/78 in 319.2 seconds. Production and Phase 6FH contract SHA-256 values were unchanged; static devlog validation passed and no Kit/CDB/GPU helper remained.

- Added the frozen Phase 6FG three-layer paired-readback contract without reclassifying Phase 6FD/6FE/6FF. Absolute resource/lifecycle/integrity conditions remain hard gates; slopes, rolling exceedances, residuals, recovery, normalized memory, Working Set, GPU memory, and occupancy correlations are now versioned warning telemetry only.
- Added the balanced independent-process `ABC / BCA / CAB` population, adjacent synchronous operation evidence, cache/shader log context, and synthetic distinctions for transient recovery, absolute/lifecycle failures, first-cache plateau, and per-iteration staircase accumulation. Repeated readback remains excluded.
- Stopped the formal population without retry at sequence 2 A after five passing conditions. Two B runs observed one-readback CPU increments of 306,528,256 and 339,447,808 bytes. Two C runs observed 302,247,936 bytes at readback and zero bytes at `numpy.asarray(fuel)`; both retained the 41,398,016-byte same-object zero-copy classification and zero weak-reference residual.
- Recorded the no-readback control's 180-second stage-close timeout as an unknown USD-context shutdown failure. Bounded CDB captured modules and a partial all-thread stack at `omni_usd!UsdManager::destroyContext`, did not match the accepted NGX signature, and timed out before detach. Exact-identity cleanup removed the observed descendants and left no residual; no full dump, fatal, access violation, or automatic upload occurred.
- Reconfirmed Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, focused Phase 6F 56/56, and the standard suite 78/78 in 314.8 seconds. Production SHA-256 remained unchanged. Phase 6FG is a lifecycle safe stop; one readback, fuel alias lifetime, and repeated readback remain unqualified.

- Added the pre-runtime Phase 6FF multi-window memory-boundedness contract without reclassifying Phase 6FD/6FE or changing the historical 8 MiB/s decision. It separates absolute ceilings, finite transient size/recovery, sustained late-window growth, normalized memory, and no-readback control deltas over a 48-second observation.
- Froze a control-first nine-process order (three no-readback controls, three C0 acquire/discard runs, then three C1 fuel-alias runs), retained the 14 GiB Kit and 16 GiB tree ceilings, and excluded repeated readback and production changes. Synthetic calibration accepts six bounded transient patterns and rejects monotonic, late, staircase, per-block, ceiling, and incomplete-shutdown failures.
- Stopped the formal Phase 6FF population at no-readback control run01 without retry. It completed 97 samples over 47.966 seconds and exited normally with material high-water recovery, but three consecutive late rolling windows exceeded the frozen diagnostic 8 MiB/s persistence level, above the predeclared maximum of two. Control runs 02/03 and all C0/C1 conditions were not started.
- Reconfirmed the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, focused Phase 6E/6F contracts 271/271, the standard suite 78/78 in 320.5 seconds, unchanged production SHA-256, and zero fatal/dump/upload/CDB/residual. The result remains a memory-boundedness safe stop rather than a one-readback qualification.

- Added the Phase 6FE lag-aware occupancy/memory contract without reclassifying Phase 6FD. Historical audit now records same-sample through four-sample response, active rebound, delayed reclaim, bounded allocator-cache retention, and global boundedness separately. Required synthetic cases accept bounded immediate/delayed/cache/periodic/constant series and reject linear, accelerating, repeated, stale, ceiling, and incomplete-shutdown cases.
- Kept an initial no-Kit report-label compatibility safe stop, fixed only that shared diagnostic label, and restarted from an empty root. Formal run01 C0 had representative startup and normal exit; the lagged event layer passed, but global Private Bytes slope was 9.292 MiB/s versus the frozen 8 MiB/s maximum. C1 and runs 2/3 were not started or retried, so one fuel-alias lifetime and repeated readback remain unqualified.
- Reconfirmed the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, focused Phase 6E/6F contracts 262/262, and the eight-process standard suite 78/78 in 317.6 seconds. Production SHA-256 remained unchanged and final fatal/dump/upload/CDB/residual counts were zero.

- Added the frozen Phase 6FD pre-readback startup-liveness gate and one-readback fuel-alias contract. It reuses the Phase 6FC order, blocks readback unless the fixed four-log source is representative by frame 60, retains delayed/small-field evidence without retry, and moves unexplained startup recurrence to low-frequency monitoring without sleep or automatic recovery.
- Ran C0 and C1 from a new empty population. Both had identical representative startup (`269 / 505 / 688 / 1118` blocks at frames 1/30/60/120), normal OS exit, and zero fatal/dump/upload/CDB/residual. C0 passed. C1 reconfirmed a 39.480 MiB same-object zero-copy fuel alias with 0-byte `numpy.asarray()` CPU/GPU delta and zero observable weak-reference residual, but failed the frozen active-drop memory-response predicate (`0.809524 > 0.75`), so the one-readback lifetime and repeated readback remain unqualified.
- Reconfirmed the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, focused contracts 90/90, and the eight-process standard suite 78/78 in 320.9 seconds. Production SHA-256 was unchanged and post-test Kit/CDB/GPU-helper residual count was zero.

- Added the frozen, production-neutral Phase 6FC startup-reproduction contract and reused the Phase 6FB shared lifecycle with per-frame source identities, explicit timeline-reset/update/play boundaries, process-gap evidence, and one-variable Flow-interface/update-order controls. An initial no-Kit launch binding defect was retained as a zero-sample safe stop and corrected independently.
- Ran six new readback-free baseline processes: all were representative with identical `269 / 505 / 688 / 1118` frame 1/30/60/120 values, so the 24-block reproduction rate was 0/6. A1 Flow-interface timing matched baseline; A2 zero stopped updates reduced initial occupancy but remained representative; A3 one extra update also remained representative. All ten formal processes exited normally with no CDB/fatal/dump/upload/residual, while production and resource ceilings remained unchanged.
- Reconfirmed the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, focused shutdown and startup contracts 82/82, and the eight-process standard suite 78/78 in 328.2 seconds. Static devlog validation passed and the production app SHA-256 remained unchanged.

- Added the production-neutral Phase 6FB startup-ingestion probe with a predeclared SHA-256 contract and flushed markers for stage connection, renderer readiness evidence, Flow-interface acquisition, timeline start, Kit update number, live Point payload identity/hash, and every frame through 120. Production behavior and all resource ceilings are unchanged.
- Audited frozen Phase 6FA evidence: the representative and 24-block processes first diverged at frame 1 before readback despite matching stage/payload/source inputs. Two new readback-free processes reproduced an identical representative trace (`269 / 505 / 688 / 1118` blocks at frames 1/30/60/120), normal OS exit, and zero fatal/dump/upload/CDB/residual. Release build, Phase 0 RTX, Phase 3, focused 25/25, and the standard 78/78 suite passed; production SHA-256 remained unchanged. The exact native startup trigger remains unconfirmed; repeated readback remains blocked.
- Recorded the Phase 6FA safe stop without changing Phase 6EY/6EZ history. An independent D1 process reproduced a fresh 24-block Flow field from frame 1 through frame 60 before readback, excluding readback, alias release, and `numpy.asarray(fuel)` as the direct initial cause; the strongest remaining boundary is Flow/Point-emitter process startup or ingestion, but the exact trigger is unconfirmed.
- Corrected the diagnostic-only public fuel helper and restarted the formal population from a new empty root. D0 passed functional and memory-stationarity gates, then exceeded the frozen 180-second stage-close timeout. Bounded CDB evidence did not match the known NGX signature, exact-identity cleanup left zero residual, and D1/D2 were not started. Production and all resource ceilings remain unchanged.
- Passed 22/22 focused contracts, the Release build, Phase 0 RTX, Phase 3 with mass-balance error 0 and active blocks final/peak 256/353, and the eight-process standard suite with 78/78 tests in 349.5 seconds. The production app SHA-256 remained unchanged and final Kit/CDB/helper residual count was zero.
- Declared the production-neutral Phase 6FA Flow functional-liveness and occupancy-aware memory contract without changing the frozen Phase 6EY/6EZ outcomes. Offline evidence proves C1 was already at 24 active blocks before readback; D0/D1/D2 now separate no readback, readback/release, and one `numpy.asarray(fuel)` while requiring fresh timeline/telemetry, positive Point input, public fuel-field evidence, unchanged identities, bounded resources, and normal exit.

- Restarted Phase 6EZ from a new empty root with no prior-sample reuse. C0 formally passed. C1 completed one `np.asarray(fuel)` and normal OS exit, demonstrating a same-object, shared-memory, zero-copy alias with no weak-reference residue, but failed the unchanged dynamic-stationarity contract because active blocks stayed fixed at 24. C1 was not retried or qualified, and repeated conversion remains blocked.
- Reconfirmed the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, 232/232 focused Phase 6 contracts, and the eight-process standard suite with 78/78 tests in 292.5 seconds. Phase 3 reported zero dry/wet mass-balance error, active blocks final/peak 244/344, and peak fuel 1.0; the production app SHA-256 remained unchanged and no Kit, CDB, or `nvidia-smi` process remained.
...
- Safely stopped `filtered_933_on` twice at the unchanged 14 GiB Kit Private Bytes guard: 15.10 GB in the initial root and 15.72 GB after bounding production-four scalar capture to one representative blocker. Exact cleanup left no residual and fatal/dump/upload/device-loss/TDR counts stayed zero.
- Froze no formal qualification contract and started neither 100% runtime, offset-zero runtime, the three-run population, nor video. Filter necessity, full-supply safety, and recommended offset remain unresolved; Phase 6ER and latest demo stay unchanged.
- Passed the Release build, Phase 0 RTX, Phase 3 with dry/wet mass-balance error 0 and Flow active blocks final/peak 274/335, 161/161 focused contracts, and the eight-process standard suite with 78/78 tests in 342.5 seconds. Production app SHA-256 remained unchanged.

- Audited the frozen Phase 6EQ `production_four` fixture and proved that its yaw-90 upper logs overlapped by 0.50 m along their shared long axis, with Mesh signed distance -0.101422 m and 480 other-log surface Point centers inside. The actual production layout does not share this defect.
- Added a separate Phase 6ER corrected four-log fixture derived from the production orientation pattern. Offline exact-Mesh gates found zero other-log Point centers, zero volume-overlap pairs, 0.23625 m parallel gaps, and only intentional crossed-layer contact within 1e-6 m, while retaining 4 x 360 Points and the existing proxy geometry.
- Reclassified no prior result. Phase 6EQ artifacts, contract, failure, and its 22 unstarted conditions remain frozen. The 0.05 m support sphere remains an engineering one-velocity-voxel assumption, not a public Flow radius.
- Measured corrected-fixture weighted supply retention of 80.00% strict, 86.67% allow-self-support, and 93.33% allow-self-center, with zero active other-log support intersections.
- Completed seven isolated scalar calibration processes and froze a separate control-relative Phase 6ER contract before formal execution. Emitterless blocker temperature/smoke were zero, while normal-source Collision ON/OFF deep sums were 7.76%/5.71% in calibration.
- Safely stopped the fresh formal population after 4/24 normal exits and 0 accepted complete populations. Deep velocity and deep scalar gates passed, but frozen temperature opposite/far and smoke opposite ratios failed; no retry, later condition, visual run, video, production integration, or latest-demo change occurred.
- Passed the Release build in 8.49 seconds, Phase 0 RTX in 23.3 seconds, Phase 3 in 31.4 seconds with dry/wet mass-balance error 0, 153/153 focused contracts, and the eight-process standard suite with 78/78 tests in 367.4 seconds. Devlog validation passed 390 references, 198 JSON, 164 SVG, and 2 ZIP files; production app SHA-256 remained unchanged.

- Added the default-off Phase 6EQ self-Collider tolerance contract and exact-Mesh Point classification. Per-point evidence now separates self/other signed distance, center-inside, support-sphere intersection, disable reason, and original/enabled fuel, temperature, and smoke without changing Point order, layout, revision, production, or Phase 6EP artifacts.
- Completed all 18 predeclared offset-sweep processes with normal OS exit. At the selected production-four offsets, strict/self-support/self-center retained 75.56%/55.83%/64.72% of weighted supply; active other-log support intersections stayed zero. Self allowance did not exceed the conservative outward baseline because other-log intersections dominate.
- Safely stopped the 24-process formal population after 2 partial-evidence processes. Collision OFF passed; the first strict Collision ON process retained 100% supply, reached active blocks 129, normal exit, and zero other-log deep velocity, but failed the frozen deep temperature/smoke gates at 1.0/1.4052734375. It was not retried, no later condition or visual capture started, and accepted formal population remains 0/24.
- Recorded a machine-readable safe-stop JSON/SVG and design/devlog entry. No comparison video is published because its prerequisite numeric population failed; the latest-demo pointer remains unchanged. Production app SHA-256 and the existing Phase 6EP report SHA-256 remain unchanged, with fatal/dump/upload/device-loss/TDR/residual counts zero.
- Passed the Release build in 8.92 seconds, 72/72 focused Phase 6EA/6EB/6EL/6EP/6EQ contracts, the eight-process standard suite with 78/78 tests in 346.2 seconds, and devlog validation for 387 references, 245 IDs, 197 JSON, 163 SVG, and 2 ZIP files. Browser rendering was unavailable because no browser was connected in this session.

- Added the independent Phase 6EN engineering-tolerance contract with schema `campfire.phase6en.static-pose-engineering-qualification-contract.v1` and SHA-256 `C6A73B07385519160488DA07C023EC5E5104BB0A8C1BDAD70D01B15327CAE1AF`. The historical `1e-5 m/s` contract and Phase 6EM formal failure remain unchanged.
- Predeclared `1e-4 m/s` deep/center hard maxima, retained the `0.1 m/s` OFF positive controls, 1% relative/stale gates, exact-Mesh depth regions, and boundary-band exclusion, and added a diagnostic-only `5e-5 m/s` warning level.
- Completed a fresh 36-process population from process 1 with no prior sample reuse. All 36 processes reached normal OS exit and all 144 velocity samples passed absolute, relative, positive-control, and stale-position gates. Worst ON deep/center were `1.5913658e-5`/`1.0030950e-5 m/s`; no deep/center cell exceeded the warning level.
- Recorded minimum OFF deep/center `7.7671523`/`5.9724069 m/s`, worst deep ratio `2.0488407e-6`, exact maximum-cell coordinates and signed distances, per-threshold counts, run variation, and reproduction of the Phase 6EM P4 11-deep/one-center locations across every new P4 sample.
- Kept production unchanged. All fatal/dump/upload/device-loss/TDR/residual and CDB counts were zero; runner/diagnostic/Kit/tree peaks were 103.8 MB/27.4 MB/12.10 GB/12.25 GB. This internal numerical Phase creates no video and does not move the latest-demo pointer.
- Passed the Release build in 6.48 seconds, Phase 0 RTX in 21.5 seconds, Phase 3 in 27.8 seconds, 114/114 focused Phase 6EA–6EN contracts, the eight-process standard suite with 78/78 tests in 333.8 seconds, and devlog validation for 374 references, 193 JSON, 159 SVG, and 2 ZIP files.

- Restarted the byte-identical Phase 6EG 36-process contract from a fifth, empty artifact root after explicit approval and with the Phase 6EL residual-only CDB path active. No prior sample was reused, and production, poses, order, thresholds, Mesh, emitter, and Flow settings stayed unchanged.
- Completed run-1 P0–P3 ON/OFF as eight normal-exit incremental-gate passes, then safely stopped at `run_1/P4_y24_z31_on`. P4 also reached functional pass, four velocity samples, active blocks 42, fuel 0.8, `shutdown_complete`, and normal OS exit, but its frozen numeric gate failed.
- Preserved the predeclared `1e-5 m/s` limit after observing P4. Deep-interior maximum was `1.5913658e-5 m/s` with 11 above-limit voxels per frame; center-axis maximum was `1.0030950e-5 m/s` with one above-limit voxel per frame. P4 OFF and later conditions were not started, so no pair ratio or stale-position predicate was evaluated.
- Kept Phase 6EG unqualified at `0/36` complete-population acceptance. CDB was not invoked because Kit exited normally within the shutdown grace; fatal, dump, upload, device-lost, TDR, resource-limit, and residual-process evidence remained zero. Production and frozen-contract SHA-256 values stayed unchanged.
- Added a machine-readable Phase 6EM safe-stop summary, numeric SVG, focused contract, design record, and devlog entry. This internal diagnostic does not create a demo or move the latest-demo pointer, and PointEmitter–CollisionProxy coexistence remains blocked.
- Passed the Release build in 8.28 seconds, Phase 0 RTX in 20.4 seconds, Phase 3 in 26.8 seconds with dry/wet mass-balance error 0 and unchanged authority hashes, 107/107 focused Phase 6E contracts, and the eight-process standard suite with 78/78 tests in 314.0 seconds. Devlog validation passed 372 references, 192 JSON, 158 SVG, and 2 ZIP files; no Kit/CDB/`nvidia-smi` process remained.
...
- Made bounded Kit-log tailing auxiliary so an exclusive lock is persisted as `log_capture_error` while the diagnostic continues to GPU inventory, CDB decision, bounded JSON commit, and cleanup. The dedicated locked-log fixture passed without claiming diagnostic success.
- Extended the resource guard to retain PID + creation-time + exact-path identities for every observed descendant. A surviving descendant now fails the run, is stopped only on exact identity match, and is rechecked. An orphan-child fixture confirmed fail-closed classification and zero residual.
- Kept all resource budgets and production behavior unchanged. P4 runner/diagnostic/Kit/tree peaks were 99.4 MB / 83.0 MB / 12.10 GB / 12.23 GB; fatal, dump, automatic upload, device loss, and TDR evidence were zero. No new demo was generated for this internal-only diagnostic.
- Passed the Release build in 7.09 seconds, 104/104 focused collision/resource/shutdown contracts, the eight-process standard suite with 78/78 tests in 310.5 seconds, and devlog validation with 368 references, 190 JSON files, and 156 SVG files. Production and frozen-contract hashes stayed unchanged; no Kit/CDB/diagnostic helper remained.

- Isolated the complete Phase 6EJ lightweight shutdown diagnostic in a bounded short-lived PowerShell child. The parent receives only a bounded JSON report or failure status; stdout/stderr go directly to files, and durable markers cover identity, capture lock, GPU inventory, log parsing, CDB decision, JSON commit, cleanup, child exit, and parent return.
- Extended the low-volume Phase 6EG resource trace with normalized process CPU telemetry. Each sample records cumulative user/kernel time, interval CPU percentage where 100% means all logical CPUs, lifecycle section, and high-CPU Kit thread IDs only above a threshold. GPU telemetry remains in the isolated low-frequency helper.
- Passed the isolated diagnostic and forced-timeout fixtures with child peaks of 85.1/71.2 MB and zero residual process. CDB is unavailable in the installed WinDbg package lookup, so a future shutdown residual still fails closed instead of being accepted without a stack signature.
- Confirmed telemetry overhead was bounded across three OFF/ON runs: runner peak changed by -0.17 MB and mean duration by +0.265 seconds. A single P0-equivalent probe completed four velocity samples, active blocks 26, fuel 0.8, `shutdown_complete`, normal OS exit, and zero fatal/dump/upload/device-loss/residual evidence.
- The P0 probe did not reproduce the Phase 6EI silent shutdown hang. During its shutdown section Kit CPU averaged 1.84% and peaked at 3.77% of all logical CPUs, consistent with low teardown activity rather than a sustained CPU spin. The original 553.9 MB parent allocation mechanism and silent-hang CPU mode therefore remain unconfirmed. Phase 6EG stays at 0/36 accepted and was not restarted.
- Reconfirmed the Release build, 91/91 focused Phase 6E contracts, the eight-process 78/78 standard suite in 303.7 seconds, and devlog validation (366 references, 189 JSON, 155 SVG). Production app and frozen-contract SHA-256 values are unchanged; no Kit, CDB, or `nvidia-smi` process remains.

- Restarted the frozen Phase 6EG qualification with explicit approval in production-neutral Phase 6EI after the isolated GPU-inventory preflight passed. The preflight measured runner 74.9 MB, `nvidia-smi` 16.0 MB, and unique tree 100.4 MB with zero timeout, limit breach, or residual process.
- Added a fail-closed incremental numeric gate after every completed process, persisted per-run physical/commit minima, and made GPU-inventory failure invalidate the lightweight shutdown diagnostic. The six poses, 36-process order, Mesh, Flow settings, frames, and thresholds remain byte-for-byte unchanged.
- Safely stopped the new formal root at active condition `run_1/P0_identity_on`. Flow sampling produced four velocity NPZ files, active blocks 26, fuel 0.8, and `shutdown_complete`, but normal OS exit was not established. After the shutdown capture lock and 210-byte isolated GPU inventory output were written, the parent runner grew from 146.9 MB to 553.9 MB and crossed its unchanged 512 MiB limit before the diagnostic report was committed.
- Rejected the process and all samples: 0/36 accepted, no retry, and no later condition started. Kit stayed below 14 GiB at 12.52 GB, the unique tree stayed below 16 GiB at 12.62 GB, available/commit headroom stayed above 90/109 GB, and fatal/dump/upload/device-lost/TDR/residual counts were zero after cleanup. Phase 6EG remains unqualified and PointEmitter–CollisionProxy coexistence remains blocked.
- Reconfirmed the Release build (8.13 seconds), Phase 0 RTX (164.4 seconds), Phase 3 authority/mass-balance/Flow input (194.1 seconds), 84/84 focused safety and collision contracts, and the eight-process 78/78 standard suite (313.8 seconds wall time). The production app SHA-256 stayed unchanged and no Kit/CDB/GPU-inventory helper remained.

- Diagnosed the Phase 6EG 512 MiB stop in production-neutral Phase 6EH. The original guard measured only the direct runner PowerShell process—not Kit or a tree aggregate. P0/P3 stage-open calibration completed with runner peaks 95.5/96.0 MB, Kit Private peaks 11.27/11.42 GB, and de-duplicated tree peaks 11.41/11.72 GB.
- Added a streamed PID/start-time resource trace with separate runner, Kit, diagnostic-child, tree, physical-memory, and commit-headroom budgets. The strict runner limit remains 512 MiB; the measured formal budgets are 14 GiB for Kit and 16 GiB for the unique process tree.
- Reproduced the runner growth after a later `timeline_stopped` stage-close hang. The trace places the first growth immediately after the lightweight diagnostic's in-process `nvidia-smi` inventory call; Kit stayed bounded. GPU inventory now runs as a 15-second/128 MiB helper with stdout/stderr directed to files and line-streamed parsing.
- Safely stopped the fresh formal matrix at 12/36 normal-exit processes when run-2 P2 OFF reached sampling and `timeline_stopped` but not stage close, then the runner crossed 512 MiB during diagnostics. No sample from either incomplete root is treated as qualification evidence, and the failed condition was not retried.
- Recorded PointEmitter–CollisionProxy coexistence as the next integration-preparation Phase only after all Phase 6EG qualification processes pass. No PointEmitter placement, production default, emitter schema, collision geometry, Flow setting, V3, Resident, or wood-authority behavior changed.
- Reconfirmed the Release build, Phase 0 RTX, Phase 3 authority/mass-balance/Flow input, 81/81 focused safety and collision contracts, and the eight-process 78/78 standard suite (311.9 seconds). The new roots contained no fatal token, dump, or upload attempt; no Kit/CDB process remained, and the production app SHA-256 stayed unchanged.
...
- Passed Release build, Phase 0 RTX, Phase 3 authority/mass-balance, 67/67 focused safety and collision contracts, and the eight-process 78/78 standard suite. Production SHA-256 remained unchanged and no Kit/CDB process, fatal, dump, or upload attempt remained.

- Added the production-neutral Phase 6EE spatial diagnostic for the Phase 6EC A/B/C stages. It stores every public NanoVDB cell in the exact authored-Mesh neighborhood as compact NPZ, records world/local coordinates, velocity components, exact low-poly surface distance, depth, face class, analytic-cylinder disagreement, and 6/18/26-neighbor connectivity without claiming access to Flow's private occupancy mask.
- Classified the Y40 residual at the existing `1e-5 m/s` gate as boundary/ROI-localized: the 0.5-1-cell band reached 0.675930 m/s, but all cells deeper than one cell and the center-axis band were at or below `8.35253e-6 m/s`. At `1e-6`, micro-velocity remains connected to about 3.05 cells, so the field is not mathematically zero.
- Found 352 B cell-records that are inside the ideal Cylinder but outside the actual 12-sided Mesh; 160 exceed `1e-5 m/s` and the maximum is 2.97754 m/s. This explains why the former analytic core ROI could report about 0.719 m/s while the exact Mesh interior beyond one cell passes the old numeric threshold. Phase 6EC remains unqualified and production is unchanged.

- Completed the fresh Phase 6EC A/B/C public NanoVDB matrix after bounding each case runner and post-exit log readiness. All three probes produced four samples, reached `shutdown_complete`, exited with code 0, and classified as functional pass / normal exit with no fatal, dump, upload, or residual process.
- Kept Phase 6EC at a numeric safe stop: the axis-aligned core velocity remained 0 m/s, while the Y40 collision-ON transformed core retained 0.479-0.719 m/s versus 7.767-9.094 m/s with collision OFF. Temperature, burn, and smoke maxima were reduced to 0.644, 0.236, and 0.250 of OFF, but the predeclared `1e-5 m/s` rotated-core velocity gate failed.
- Did not generate the visual matrix or change the latest demo because numeric qualification failed. Production remained unchanged, and arbitrary rotation, RenderSurface, PhysX sharing, dynamic transforms, and 20-log integration remain blocked.
- Passed Release build in 7.09 seconds, Phase 0 RTX in 19.4 seconds, Phase 3 with dry/wet mass balance error 0 and unchanged authority hashes, the eight-process standard suite with 78/78 tests in 309.8 seconds, and devlog validation with no missing reference, malformed JSON/SVG, duplicate ID, or replacement character.
...
- Safely stopped the next fresh root at A when its normally exited Kit log was transiently unreadable and Phase 6ED correctly failed closed. Added a bounded post-exit log-readiness wait to the shared diagnostic case runner; exhaustion remains evidence-unavailable and cannot be accepted as no exception.
- Retained a further fresh root outside the formal population after A required 4.33 seconds for log readiness and B exceeded the initial five-second window despite an `ok` probe, OS exit 0, and a subsequently readable exception-free log. Increased only that bounded readiness window to 15 seconds; no matcher or classification rule was relaxed.
...
- Froze the Phase 6FX fresh nine-process memory-ceiling qualification without reclassifying Phase 6FT/6FV or starting Phase 6FO. The shared Phase 6FV execution engine is now phase-parameterized while preserving its default behavior; Phase 6FX adds the Phase 6FW fail-closed final identity classifier and keeps Phase 6FU exact cleanup and Phase 6FS release-after-close unchanged.
- Fixed the balanced M0/M1/M2 population, 14 GiB telemetry threshold, 16 GiB Kit and 17 GiB tree absolute stops, 15.5 GiB normal-peak qualification maximum, and 512 MiB candidate headroom before runtime. Production, defaults, Flow, Point payload, CollisionProxy, V3, readback, capture, video, and Phase 6FO remain unchanged.
- Safely stopped at attempt06 M0 after five representative normal exits. The readback-free control timed out at stage close after 180.016801 seconds with Kit/tree peaks of 14,851,428,352/15,014,383,616 bytes; no later lifecycle marker or normal OS exit was claimed, and attempts07--09 were not launched.
- Preserved partial memory evidence without qualifying a ceiling. Five normal peaks were 14,534,746,112--14,967,640,064 bytes, leaving only 64,745,472 bytes below old 14 GiB, but the incomplete population means Kit 16 GiB, tree 17 GiB, and Phase 6FO restart remain unqualified.
- Bounded CDB stack and module passes timed out without attach/native frames; explicit detach and exact cleanup succeeded. The failure is not the accepted NGX signature and remains unknown native lifecycle. Phase 6FW totals were owned residual 0, unknown 0, wrong mismatch stops 0, and final Kit/CDB/GPU-helper residual 0.
- Reconfirmed the safe stop with Release build, Phase 0 RTX, Phase 3 zero dry/wet mass-balance error and wood-owned Flow input, 191/191 focused Phase 6F contracts, and the standard 8-process 78/78 suite. Production SHA-256, defaults, video, and latest demo remained unchanged.
# Phase 6FY

- Froze a new three-axis memory qualification contract without reclassifying Phase 6FT, 6FV, or 6FX. Memory validity, stage-close lifecycle, and bounded diagnostic/exact cleanup are evaluated independently; a post-measurement stage-close timeout remains a formal lifecycle failure while its durably committed pre-close peak remains in the memory distribution.
- Added a bounded pre-close streaming committer, twenty synthetic/short-lived fixtures, a maximum-two replacement policy that never overwrites the original timeout, and separate complete/partial CDB classifications. The basic M0/M1/M2 population remains nine fresh processes with at most two additional timeout replacements; Phase 6FO, production, defaults, readback, capture, video, and P4 remain stopped.
